...
- **ESP32-S3 の 3 板は WPA3 移行モード（WPA2/WPA3 混在）の AP に STA 接続できません。**
  ...
  **さらに、この切断のあとスケッチが止まります**（2026-09-20 実測）。
  `reason=17` で切れたあと `loop()` が呼ばれなくなり、コンソールも黙ります
  （CPU は動いています——JTAG で PC は進んでおり、止まっているのは
  Arduino タスクです）。M5CoreS3 で 120 秒採取して 3 周で停止、
  M5AtomS3 Lite の 2026-09-19 の採取も同じ形で終わっていました。
  **`WiFi` 構成を選び、繋がらない AP へ `begin()` する S3 のスケッチは、
  そのまま無反応になります。**原因は未特定で、この AP しか手元にないため
  「reason=17 のとき」以上に絞れていません。
