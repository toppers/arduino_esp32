...
#
#  Stage the sketch-independent FMP3 artifacts of the ESP32-P4 (cmake -P).
#
#  The P4 copy of prebuilt_stage_c5.cmake (StampP4 plan stage A1), kept as a
#  separate file for the same reason that one is separate from the C6 and
#  Xtensa scripts: those are under the X-check and stay textually
#  unchanged. What this copy changes, beyond the chip name and this header:
#    - ROM_LDS may be empty (step (6)): the P4 minimal stage links no ROM
#      linker script (the chip layer is self-contained), and the manifest's
#      romLinkerScripts is then [] - the driver takes an empty list.
#    - the XIP linker script's INCLUDE fragments are inlined into the staged
#      copy (step (4)): esp32p4_xip.ld reads p4_mem_high.ld and three more
#      beside it, which GNU ld finds through the current directory or -L
#      only; the sketch link on the user's machine has neither, so the
#      shipped ld/esp32p4_xip.ld carries the fragments' text in place.
#    - flashSize 16MB, flashMode qio (the M5Stamp-P4's flash and the
#      board's build.flash_mode; the C5 script says 4MB / dio).
#  The image conditions the driver checks for this chip (C-1 to C-9 with
#  the esp32p4 row of FIXED_VMA_LAYOUTS: chip_id 0x0012, the two flash
#  segments, the loader segment at 0x4ff2cbd0) are checked at link time,
#  not here - the stage carries no image. Same purpose: cfg is fixed
...
#    - No flash_cache_init.o. The C5 image is linked at the virtual addresses
...
#      per-chip table here.
#    - flashSize is 16MB and flashMode qio: the M5Stamp-P4 (M5Stack
#      boards.txt m5stack_stamp_p4.build.flash_size / build.flash_mode).
#
...
#          -DPROFILE=minimal -DA1_CHIP=esp32p4 -DXIP_LD=... -DROMLD_ROOT=...
...
#          -P cmake/prebuilt_stage_p4.cmake
#
...
if(DEFINED CONSUMER_OBJS_FILE AND NOT CONSUMER_OBJS_FILE STREQUAL "")
  if(NOT EXISTS "${CONSUMER_OBJS_FILE}")
    message(FATAL_ERROR
      "prebuilt_stage_p4: CONSUMER_OBJS_FILE does not exist: ${CONSUMER_OBJS_FILE}")
  endif()
  file(STRINGS "${CONSUMER_OBJS_FILE}" CONSUMER_OBJS)
  list(REMOVE_ITEM CONSUMER_OBJS "")
  if(CONSUMER_OBJS STREQUAL "")
    message(FATAL_ERROR
      "prebuilt_stage_p4: CONSUMER_OBJS_FILE is empty: ${CONSUMER_OBJS_FILE}")
  endif()
endif()
...
foreach(v STAGE_DIR AR GCC LIBFMP3 CONSUMER_OBJS PROFILE XIP_LD ROMLD_ROOT ROM_LDS
  ...
  if(NOT DEFINED ${v})
    message(FATAL_ERROR "prebuilt_stage_p4: required argument ${v} is undefined")
  endif()
endforeach()

if(NOT DEFINED A1_CHIP OR A1_CHIP STREQUAL "")
  message(FATAL_ERROR "prebuilt_stage_p4: A1_CHIP is required (esp32p4)")
endif()
if(NOT A1_CHIP STREQUAL "esp32p4")
  message(FATAL_ERROR
    "prebuilt_stage_p4: esp32p4 only (got ${A1_CHIP}); the esp32c6 uses prebuilt_stage_c6.cmake, the Xtensa chips prebuilt_stage.cmake")
endif()
if(LINK_BASE_FLAGS STREQUAL "")
  ...
  message(FATAL_ERROR "prebuilt_stage_p4: LINK_BASE_FLAGS is empty")
endif()
...
function(stage_claim name src)
  ...
  if(NOT _idx EQUAL -1)
    ...
    message(FATAL_ERROR
      "prebuilt_stage_p4: basename collision: '${name}'\n  existing: ${_prev}\n  new: ${src}\n"
...
if(NOT _rc EQUAL 0)
  message(FATAL_ERROR "prebuilt_stage_p4: ar x failed (rc=${_rc})\n${_err}")
endif()
...
foreach(m ${_members})
  ...
  if(_nn STREQUAL _bn)
    message(FATAL_ERROR "prebuilt_stage_p4: unexpected member name ${_bn}")
  endif()
...
foreach(o ${CONSUMER_OBJS})
  ...
  if(_nn STREQUAL _bn)
    message(FATAL_ERROR "prebuilt_stage_p4: unexpected consumer object name ${_bn}")
  endif()
  stage_claim("${_nn}" "${o}")
...
if(EXISTS "${OBJDIR}/cfg1_out.o")
  message(FATAL_ERROR "prebuilt_stage_p4: cfg1_out.o was staged (it must not be linked)")
endif()
...
if(NOT DEFINED STRIP_DEBUG OR STRIP_DEBUG)
  ...
  if(NOT DEFINED OBJCOPY OR NOT EXISTS "${OBJCOPY}")
    ...
    if(NOT OBJCOPY)
      message(FATAL_ERROR
        "prebuilt_stage_p4: ${_objcopy_name} was not found "
  ...
  foreach(o ${_to_strip})
    ...
    execute_process(COMMAND "${OBJCOPY}" --strip-debug "${o}"
                    RESULT_VARIABLE _rc ERROR_VARIABLE _err)
    if(NOT _rc EQUAL 0)
      message(FATAL_ERROR "prebuilt_stage_p4: strip failed ${o} (rc=${_rc})\n${_err}")
    endif()
    file(SIZE "${o}" _sz)
  ...
  message(STATUS
    "prebuilt_stage_p4: stripped debug info (${_before_kb} KB -> ${_after_kb} KB)")
  set(_debug_info "stripped")
else()
  set(_debug_info "full")
endif()

#  ---- (4) linker script, with its INCLUDE fragments inlined ----
#
#  esp32p4_xip.ld says `INCLUDE p4_mem_high.ld` (and p4_iram_extra.ld,
#  p4_dram_extra.ld, p4_bss_high.ld), the dev repository's hook for the
#  configurations that need more RAM regions; the minimal stage reads the
#  comment-only fragments beside the script. GNU ld resolves INCLUDE
#  against the current directory and -L, neither of which the driver
#  points at the stage's ld/, so the staged copy gets each fragment's text
...
string(REGEX MATCHALL "\n[ \t]*INCLUDE[ \t]+[A-Za-z0-9_./-]+\\.ld" _xip_includes "${_xip_text}")
foreach(_inc IN LISTS _xip_includes)
  string(REGEX REPLACE "^\n[ \t]*INCLUDE[ \t]+" "" _frag "${_inc}")
  if(NOT EXISTS "${_xip_ld_dir}/${_frag}")
    message(FATAL_ERROR
      "prebuilt_stage_p4: ${_xip_ld_name} INCLUDEs ${_frag}, which is not beside it (${_xip_ld_dir})")
  endif()
  file(READ "${_xip_ld_dir}/${_frag}" _frag_text)
  string(REPLACE "${_inc}"
         "\n/* prebuilt_stage_p4: INCLUDE ${_frag} inlined from the target directory */\n${_frag_text}"
         _xip_text "${_xip_text}")
endforeach()
if(_xip_text MATCHES "\n[ \t]*INCLUDE[ \t]+[A-Za-z0-9_./-]+\\.ld")
  message(FATAL_ERROR "prebuilt_stage_p4: an INCLUDE survived inlining in ${_xip_ld_name}")
endif()
file(WRITE "${LDDIR}/${_xip_ld_name}" "${_xip_text}")
...
if(_objlist STREQUAL "")
  message(FATAL_ERROR "prebuilt_stage_p4: nothing to link")
endif()
set(_order "")
foreach(o ${_objlist})
  get_filename_component(_bn "${o}" NAME)
...
foreach(_d ${_stage_lib_dirs})
  if(NOT IS_DIRECTORY "${_d}")
    message(FATAL_ERROR "prebuilt_stage_p4: STAGE_LIB_DIRS entry does not exist: ${_d}")
  endif()
  file(MAKE_DIRECTORY "${STAGE_DIR}/lib")
  file(GLOB _dir_archives "${_d}/*.a")
  if(_dir_archives STREQUAL "")
    message(FATAL_ERROR "prebuilt_stage_p4: no .a in ${_d}")
  endif()
  foreach(a ${_dir_archives})
    get_filename_component(_an "${a}" NAME)
    if("${_an}" IN_LIST _staged_archives)
      message(FATAL_ERROR
        "prebuilt_stage_p4: archive basename collision in lib/: ${_an} (from ${_d})")
    endif()
...
#  Empty is allowed for the P4 (see the header): the minimal stage links
#  no ROM script.
string(REPLACE "@@" ";" _romld_list "${ROM_LDS}")
list(REMOVE_ITEM _romld_list "")
foreach(l ${_romld_list})
  if(NOT EXISTS "${ROMLD_ROOT}/${l}")
    message(FATAL_ERROR "prebuilt_stage_p4: ROM linker script is missing: ${ROMLD_ROOT}/${l}")
  endif()
endforeach()

#  The board's flash, not the chip's maximum: M5Stamp-P4 = 16 MB, qio
#  (boards.txt m5stack_stamp_p4.build.flash_size / .build.flash_mode).
...
if(NOT DEFINED DUPSYM_AUDIT OR DUPSYM_AUDIT)
  ...
  if(NOT PYTHON_EXECUTABLE)
    message(FATAL_ERROR
      "prebuilt_stage_p4: the duplicate-symbol audit needs python "
  ...
  if(NOT TARGET_NM)
    message(FATAL_ERROR "prebuilt_stage_p4: ${_nm_name} was not found (duplicate-symbol audit)")
  endif()
  ...
  if(NOT EXISTS "${_repo_root}/scripts/audit_duplicate_symbols.py")
    message(FATAL_ERROR
      "prebuilt_stage_p4: audit script was not found: "
  ...
  if(NOT _dup_rc EQUAL 0)
    message(FATAL_ERROR "prebuilt_stage_p4: ${_dup_err}")
  endif()
endif()

message(STATUS "prebuilt_stage_p4: ${PROFILE} objects=${_order_count} -> ${STAGE_DIR}")
