...
#
#  Stage the sketch-independent FMP3 artifacts of the ESP32-C5 (cmake -P).
#
#  The C5 copy of prebuilt_stage_c6.cmake (C5 plan A2), kept as a separate
#  file for the same reason that one is separate from the Xtensa
#  prebuilt_stage.cmake: the C6 file is under the X-check and stays
#  textually unchanged. Apart from the chip name, the flash size comment and
#  this header, the two files are the same text; the image conditions the C5
#  adds (C-9: chip_id 0x0017 and the chip revision window) are checked by
#  the driver (scripts/fmp3_link.py FIXED_VMA_LAYOUTS["esp32c5"]) at link
#  time, not here - the stage carries no image. Same purpose: cfg is fixed
...
#      per-chip table here.
#    - flashSize is 4MB: the M5Stamp-C5 has a 4 MB flash (M5Stack boards.txt
#      m5stack_stamp_c5.build.flash_size).
#
...
#          -DPROFILE=minimal -DA1_CHIP=esp32c5 -DXIP_LD=... -DROMLD_ROOT=...
...
#          -P cmake/prebuilt_stage_c5.cmake
#
...
if(DEFINED CONSUMER_OBJS_FILE AND NOT CONSUMER_OBJS_FILE STREQUAL "")
  if(NOT EXISTS "${CONSUMER_OBJS_FILE}")
    message(FATAL_ERROR
      "prebuilt_stage_c5: CONSUMER_OBJS_FILE does not exist: ${CONSUMER_OBJS_FILE}")
  endif()
  file(STRINGS "${CONSUMER_OBJS_FILE}" CONSUMER_OBJS)
  list(REMOVE_ITEM CONSUMER_OBJS "")
  if(CONSUMER_OBJS STREQUAL "")
    message(FATAL_ERROR
      "prebuilt_stage_c5: CONSUMER_OBJS_FILE is empty: ${CONSUMER_OBJS_FILE}")
  endif()
endif()
...
foreach(v STAGE_DIR AR GCC LIBFMP3 CONSUMER_OBJS PROFILE XIP_LD ROMLD_ROOT ROM_LDS
  ...
  if(NOT DEFINED ${v})
    message(FATAL_ERROR "prebuilt_stage_c5: required argument ${v} is undefined")
  endif()
endforeach()

if(NOT DEFINED A1_CHIP OR A1_CHIP STREQUAL "")
  message(FATAL_ERROR "prebuilt_stage_c5: A1_CHIP is required (esp32c5)")
endif()
if(NOT A1_CHIP STREQUAL "esp32c5")
  message(FATAL_ERROR
    "prebuilt_stage_c5: esp32c5 only (got ${A1_CHIP}); the esp32c6 uses prebuilt_stage_c6.cmake, the Xtensa chips prebuilt_stage.cmake")
endif()
if(LINK_BASE_FLAGS STREQUAL "")
  ...
  message(FATAL_ERROR "prebuilt_stage_c5: LINK_BASE_FLAGS is empty")
endif()
...
function(stage_claim name src)
  ...
  if(NOT _idx EQUAL -1)
    ...
    message(FATAL_ERROR
      "prebuilt_stage_c5: basename collision: '${name}'\n  existing: ${_prev}\n  new: ${src}\n"
...
if(NOT _rc EQUAL 0)
  message(FATAL_ERROR "prebuilt_stage_c5: ar x failed (rc=${_rc})\n${_err}")
endif()
...
foreach(m ${_members})
  ...
  if(_nn STREQUAL _bn)
    message(FATAL_ERROR "prebuilt_stage_c5: unexpected member name ${_bn}")
  endif()
...
foreach(o ${CONSUMER_OBJS})
  ...
  if(_nn STREQUAL _bn)
    message(FATAL_ERROR "prebuilt_stage_c5: unexpected consumer object name ${_bn}")
  endif()
  stage_claim("${_nn}" "${o}")
...
if(EXISTS "${OBJDIR}/cfg1_out.o")
  message(FATAL_ERROR "prebuilt_stage_c5: cfg1_out.o was staged (it must not be linked)")
endif()
...
if(NOT DEFINED STRIP_DEBUG OR STRIP_DEBUG)
  ...
  if(NOT DEFINED OBJCOPY OR NOT EXISTS "${OBJCOPY}")
    ...
    if(NOT OBJCOPY)
      message(FATAL_ERROR
        "prebuilt_stage_c5: ${_objcopy_name} was not found "
  ...
  foreach(o ${_to_strip})
    ...
    execute_process(COMMAND "${OBJCOPY}" --strip-debug "${o}"
                    RESULT_VARIABLE _rc ERROR_VARIABLE _err)
    if(NOT _rc EQUAL 0)
      message(FATAL_ERROR "prebuilt_stage_c5: strip failed ${o} (rc=${_rc})\n${_err}")
    endif()
    file(SIZE "${o}" _sz)
  ...
  message(STATUS
    "prebuilt_stage_c5: stripped debug info (${_before_kb} KB -> ${_after_kb} KB)")
...
if(_objlist STREQUAL "")
  message(FATAL_ERROR "prebuilt_stage_c5: nothing to link")
endif()
set(_order "")
foreach(o ${_objlist})
  get_filename_component(_bn "${o}" NAME)
...
foreach(_d ${_stage_lib_dirs})
  if(NOT IS_DIRECTORY "${_d}")
    message(FATAL_ERROR "prebuilt_stage_c5: STAGE_LIB_DIRS entry does not exist: ${_d}")
  endif()
  file(MAKE_DIRECTORY "${STAGE_DIR}/lib")
  file(GLOB _dir_archives "${_d}/*.a")
  if(_dir_archives STREQUAL "")
    message(FATAL_ERROR "prebuilt_stage_c5: no .a in ${_d}")
  endif()
  foreach(a ${_dir_archives})
    get_filename_component(_an "${a}" NAME)
    if("${_an}" IN_LIST _staged_archives)
      message(FATAL_ERROR
        "prebuilt_stage_c5: archive basename collision in lib/: ${_an} (from ${_d})")
    endif()
...
if(_romld_list STREQUAL "")
  message(FATAL_ERROR "prebuilt_stage_c5: ROM_LDS is empty")
endif()
foreach(l ${_romld_list})
  if(NOT EXISTS "${ROMLD_ROOT}/${l}")
    message(FATAL_ERROR "prebuilt_stage_c5: ROM linker script is missing: ${ROMLD_ROOT}/${l}")
  endif()
endforeach()

#  The board's flash, not the chip's maximum: M5Stamp-C5 = 4 MB.
set(_flash_size "4MB")
...
if(NOT DEFINED DUPSYM_AUDIT OR DUPSYM_AUDIT)
  ...
  if(NOT PYTHON_EXECUTABLE)
    message(FATAL_ERROR
      "prebuilt_stage_c5: the duplicate-symbol audit needs python "
  ...
  if(NOT TARGET_NM)
    message(FATAL_ERROR "prebuilt_stage_c5: ${_nm_name} was not found (duplicate-symbol audit)")
  endif()
  ...
  if(NOT EXISTS "${_repo_root}/scripts/audit_duplicate_symbols.py")
    message(FATAL_ERROR
      "prebuilt_stage_c5: audit script was not found: "
  ...
  if(NOT _dup_rc EQUAL 0)
    message(FATAL_ERROR "prebuilt_stage_c5: ${_dup_err}")
  endif()
endif()

message(STATUS "prebuilt_stage_c5: ${PROFILE} objects=${_order_count} -> ${STAGE_DIR}")
