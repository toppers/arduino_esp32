/* asp3_esp_idf 6471444 esp/c6/sdkconfig_stub/sdkconfig.h の複製 (2026-09-13). */
/* 段1 では hal/systimer_ll.h / hal/usb_serial_jtag_ll.h が読む CONFIG_* のみ意味を持つ. */
/* 出典との差はこの 3 行だけ (diff で確認可能). */
/*
...
/*
 *  ------------------------------------------------------------------
 *  ★供給移行（.steering/20260716-c3c5c6-esp-idf-supply-migration）で
 *  リポジトリへ vendor したファイル。
 *
 *  出自：esp-hal-3rdparty（hal submodule．rev b90b1837）の
 *        `nuttx/esp32c6/include/sdkconfig.h` からの **verbatim コピー**
 *        （Espressif Apache-2.0 ヘッダ含め無改変．本コメントのみ付加）。
 *
 *  存在理由：sdkconfig.h は本来 ESP-IDF の Kconfig 生成物であり，
 *  **esp-idf のリポジトリ・チェックアウトには存在しない**（ビルド時に
 *  生成される）。一方 hal は NuttX 統合用の静的スタブを同梱しており，
 *  C6 ターゲットは従来これを直接 include していた（`${ESP_HAL_DIR}/
 *  nuttx/esp32c6/include`）。供給を esp-idf submodule へ移す（＝hal 参照
 *  ゼロ）にあたり，この 1 ファイルだけは写像先が無いため vendor する
 *  （PROMPT.md §4「$HAL 由来の sdkconfig.h / hal_stub 系はリポジトリへ
 *  vendor し，incflags の esp-hal-3rdparty 参照を撤去」に従う）。
 *
 *  ★C5 は hal が C5 用 NuttX ファイルを同梱していないため
 *  `esp32c5_espidf/sdkconfig_stub/sdkconfig.h`（278行）を手書きしたが，
 *  C6 は hal に既存の完全版（1155行）があるためそれを写した＝
 *  **既存ビルドと CONFIG_* が 1 ビットも変わらない**ことを優先した
 *  （供給移行で「動いていた設定」を同時に変えないため）。
 *  ------------------------------------------------------------------
 */
...
/*  段4 Task 4 fix round 1（2026-09-14）: CONFIG_ESP_PHY_ENABLE_USB は **定義しない**。
 *  esp_phy/Kconfig:113-115 は `depends on SOC_WIFI_PHY_NEEDS_USB_WORKAROUND` で、
 *  この cap は soc/esp32c3・esp32s3 の soc_caps.h だけが定義し C6 には無い
 *  （= IDF の C6 ビルドでは USJ コンソールでも unset）。Task 4 で一度足したが、
 *  対照（足しても真cold は無音のまま / 外しても真cold で全行採れる、
 *  .steering/20260913-c6-stage4/logs/task4-cold-usb1.log と task4-ctl-nousb-cold1.log）
 *  で効果が無く、真cold の無音の原因は USB-Serial/JTAG の送信武装だった。
 *  IDF の C6 既定（unset）に揃える。 */
...
#define CONFIG_BT_LE_ACL_BUF_COUNT 24
#define CONFIG_BT_LE_ACL_BUF_SIZE 255
...
#define CONFIG_BT_LE_CRYPTO_STACK_MBEDTLS 0
...
#define CONFIG_BT_LE_LL_DUP_SCAN_LIST_COUNT 100
...
#define CONFIG_BT_LE_DFT_TX_POWER_LEVEL_DBM_EFF 11
...
#define CONFIG_BT_CTRL_DFT_TX_POWER_LEVEL_EFF CONFIG_BT_LE_DFT_TX_POWER_LEVEL_DBM_EFF
#define CONFIG_BT_CTRL_COEX_PHY_CODED_TX_RX_TLIM_EFF CONFIG_BT_LE_COEX_PHY_CODED_TX_RX_TLIM_EFF
#define CONFIG_BT_CTRL_COEX_PHY_CODED_TX_RX_TLIM_DIS CONFIG_BT_LE_COEX_PHY_CODED_TX_RX_TLIM_DIS

#endif /* CONFIG_ESPRESSIF_BLE */
