 ...
 *  esp_os_enter_critical{,_isr,_safe}() を portENTER_CRITICAL() ではなく
 *  **vPortEnterCritical()** へ直接展開する（:232/:290/:349）。S3 では
 *  OS_SPINLOCK==1 なので上の portENTER_CRITICAL 写像を通るが、C6 では
 *  通らず、esp/wifi/hal_src/phy_init.c の phy_enter/exit_critical が
 *  未宣言の vPortEnterCritical を呼ぶ形になる（実測: 段4 Task 3 の初回
 *  リンクで phy_init.c.obj に `U vPortEnterCritical`。最終 ELF では
 *  phy_enter_critical が gc されて隠れていた）。実体は
 *  esp/shim/esp_shim_blobglue.c の C6 分岐（esp_shim_bt_enter/exit_critical(NULL)
 *  への委譲＝S3 と同じ写像先）。TOPPERS_ESP32C6 は上の <platform/os.h> が
 *  <kernel.h> を取り込むので、ここで見える。
 */
#ifndef TOPPERS_MACRO_ONLY
