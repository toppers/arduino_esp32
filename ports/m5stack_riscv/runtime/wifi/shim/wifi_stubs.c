 ...
 *
 *  訂正（2026-07-17、実測）：旧コメントは「S3ではROM/newlib経路で解決される」
 ...
 *  →LoadProhibited で落ちていた（.steering/20260716-mtrans2/evidence-12）。
 *
...
/* ---- esp_timer ----
 * 2026-07-27 レビュー esp-2：ここに在った「ESP_OKを返すno-op」
 * （esp_timer_create/start_periodic/stop/delete）を削除した。
 * 実体は esp/shim/esp_timer_shim.c（固定プール＋ESP_TIMER_TSK）が
 * Wi-Fi構成・BT/BLE構成の両方へ供給する。
 *
 * no-opのままだと esp/wifi/hal_src/phy_common.c の phy_track_pll_init()
 * が ESP_ERROR_CHECK 付きで成功を受け取りながら PHY PLL トラッキングが
 * 一度も走らない（S3-wifi構成。LX6-wifiは phy_init.c の
 * `#if !CONFIG_IDF_TARGET_ESP32`、S3-BLEは -DCONFIG_ESP_PHY_DISABLE_PLL_TRACK=1
 * により呼出し経路が無い）。またBLE構成では bt_shim.o の本物と
 * 多重定義になり、-Wl,--allow-multiple-definition ＋リンク行の
 * オブジェクト辞書順で本物が勝つ、という偶然に依存していた。 */
