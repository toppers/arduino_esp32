 ...
 *
 *  背景（.steering/20260710-m5-arduino-idf-mode/design.md §7 変更点①）：
 ...
 *      同じ運用）。
 *
 *    - C++コンシューマ（1-4 で実測・追加）：<kernel.h>→<t_stddef.h> は
 ...
 *    - キュー：      esp_shim_ring_create / send / recv / delete（esp_shim_ring.h）
...
/* キュー: `esp_shim_queue_*` は 2026-08-15（BL-G-3 段3）に撤去した。
 * 写像先は `esp/shim/esp_shim_ring.h` の `esp_shim_ring_*`（ISR 安全）。 */
...
/* タスク */
/*  2026-08-04（段4）: コア指定つきの `esp_shim_task_create_pinned()` と
 *  `ESP_SHIM_TASK_NO_AFFINITY` は **C 側（`esp_shim.h`）にだけ**置いた。
 *  C++ コンシューマ（M5GFX/M5Unified）はコア固定を要求しない——
 *  `m5/compat/freertos/task.h` の `xTaskCreatePinnedToCore` は
 *  `(void) xCoreID` のままである（段4 AC の §2「入れない」。理由は
 *  `.steering/20260804-dcre-stage4-tsk/RESULT.md`）。 */
