 ...
 *
 *  ========================================================================
 *  何が変わったか（2026-08-04・段4）
 *  ========================================================================
 *  従来は `esp_shim.cfg` の `CRE_TSK(SHIM_TSK1..8)`（構成により 6／8 本が有効）を
 *  静的プールとして持ち、`esp_shim.c` の `tskid_tbl[]` が `shim_tsk[]` スロットと
 *  同じ添字で ID を配っていた。**本 TU はその ID プールを廃し、カーネルの
 *  動的生成サービスコール `acre_tsk` / `mact_tsk` / `del_tsk` を直接使う。**
 *
 *  **静的版と動的版の分岐は無い**（`#ifdef` も実行時分岐も）。無条件に動的生成。
 *    理由は `1765759`（キュー写像先の分岐を廃止）と、その分岐が実際に H4 を
 *    再発させた `.steering/20260803-h4-stale-ar-hypothesis/`。
 ...
 *  **タスクの生成/削除サイクルは実在する**（実測。単一 boot でタスク生成 7 回・
 *  同一 `tskid` が 6 回再配布。発生源は M5Unified `Speaker`/`Mic` の `begin()`/`end()`。
 *  `.steering/20260804-dynamic-object-integration/object-lifetime-survey.md`）。
 *  **回数の上限が静的に決まらない**ので、案A（`stk == NULL`）では
 *  **足りる `DEF_MPK` の値が存在しない**（1 boot で 16 KB がアリーナから消える見積り。
 *  DESIGN-MEMO §4-4e (b)）。
 *
 ...
 *  ------------------------------------------------------------------------
 *  【選んだ】**`shim_tsk_stack[ESP_SHIM_NUM_TSK][ESP_SHIM_TSK_STKSZ]`（16 整列）**。
 *   これは**旧 cfg が `_kernel_stack_SHIM_TSK1..8` として置いていたものと
 *     同じ本数・同じサイズ**である。⇒ **DRAM 予算が静的に決まる性質を保つ。**
 *   1. **DESIGN-MEMO §4-4c が案B に期待したのはまさにこれ**——
 *      「§4-2 の境界（誰が**個数**を決めるか）を、そのまま『誰が**メモリ量**を決めるか』へ
 *      延長しない」。外し続けたのは**個数**の推測であって、DRAM 予算ではない。
 *   2. **新しい失敗モードが 1 つも増えない。**旧構造と使うメモリはバイト単位で同じで、
 *      生成が失敗する条件も同じ（スロット枯渇／要求 > 枠）である。
 *   3. **整列を構造で満たせる**——`__attribute__((aligned(16)))` と
 *      `ROUND16(ESP_SHIM_TSK_STKSZ)` で `CHECK_STACK_ALIGN(16)` /
 *      `CHECK_STKSZ_ALIGN(16)` の両方を静的に満たす（下記「整列」）。
 *
 *  【却下した(i)】**シムヒープ（`esp_shim_malloc`）から要求ちょうど**
 *      ——**段3（dtq）が選んだ手**である。同じ手を採らなかった理由は 4 つとも実測:
 *       ・**サイズの桁が違う。**段3 の `dtqmb` は最大 **800 B**（実測）だったが、
 *         タスクスタックは **4,096〜8,192 B**（実測: wifi 6656 / btController 4096 /
 *         nimble_host 4096 / audio 2304。`.steering/20260804-blobpool-stack/`）。
 *       ・**ヒープの高水位を測っていない。**`ESP_SHIM_HEAP_STATS` は opt-in（既定 OFF）で、
 *         `.steering/` 配下の全ログに**高水位の記録が 1 件も無い**（あるのは
 *         `heapfree=` 8 行＝**生成時点**の残量 88,784〜92,768 B だけ）。
 *         ⇒ **「入る」と言える根拠が無い。**
 *       ・**静的に証明できる予算を実行時の予算へ変換することになる。**
 *         旧構造では「スタックは必ず在る」がリンク時に決まっていた。ヒープへ移すと
 *         **Wi-Fi blob の作業メモリと競合**し、足りるかは実機でしか分からない。
 *         本タスクは**ビルドのみ・実機なし**なので、確かめずに移すのは順序が逆である。
 *       ・**「要求ちょうど」にならない。**シムヒープの払い出しは **8 整列**だが
 *         `CHECK_STACK_ALIGN` は **16** を要求する ⇒ 過剰確保＋自前整列が要る。
 *      ⇒ **却下は「ヒープが悪い」ではなく「測っていないものへ移さない」である。**
 *        ヒープへ移せば `.bss` が **24〜64 KB** 減る（＝旧静的スタックの総量）。
 *          **その利得は実在する。**取っていないのは、実機で確かめる手段が
 *          このタスクに無いからである（DESIGN-MEMO §8-B の 3 として残す）。
 *
 *  【却下した(ii)】**案A（`stk == NULL` ＋ `DEF_MPK`）**。
 *      上記のとおり bump アロケータで単調に痩せ、**足りる容量が存在しない**。
 *      DESIGN-MEMO §4-4e の決着そのもの。
 *
 *  **「現行と同じにした」部分がある**ことを隠さない——**メモリの量と置き場所の性質は
 *    旧構造と同じ**である。**本段が消したのは「個数を 3 箇所に書く」構造**
 *    （cfg の `CRE_TSK` 8 行・C 配列 `tskid_tbl[]`・マクロ `ESP_SHIM_NUM_TSK`）であって、
 *    **DRAM の使い方ではない。**これを「スタック管理を設計した」とは書かない。
 *
 *  ------------------------------------------------------------------------
 ...
 *
 *  ========================================================================
 *  自己削除と回収（DESIGN-MEMO §7-3b。本段の核心）
 *  ========================================================================
 ...
 *     `esp_shim.c` をリンクする 7 構成はすべて `TNUM_PRCID == 1`（実測）だが、
 *     **SMP のシムを作る人はここを読むこと。**
 *
 *  ========================================================================
 *  コア固定（DESIGN-MEMO §3。論点 2）
 *  ========================================================================
 ...
 *  （§3-2(c)。`esp-idf/components/esp_driver_spi/src/gpspi/spi_master.c` の `nomem:` と同じ形）。
 *  `acre_tsk` が成功して返した ID にしか `mact_tsk` を撃たない（§3-2(d)。
 *   未生成スロット ID への `mact_tsk` は `E_PAR` になる既知の穴がある＝§7-2）。
 *
 *  ========================================================================
 *  `TA_FPU`（論点 3。DESIGN-MEMO §7-7 の前提はこのリポジトリでは成り立たない）
 *  ========================================================================
 *  §7-7 は「FreeRTOS API に FPU 属性が無いので**全動的タスクに `TA_FPU` を付けるしかない**」
 *  と書いているが、**現行の静的定義はそうなっていない**（実測）:
...
#ifdef W1N3_PROBE
	/*
	 *  N-3（.steering/20260805-w1n3-preflight/AC.md 判定 7）: blob が渡した
...
esp_shim_tsk_create(uint_t slot, TASK task, EXINF exinf, PRI itskpri)
{
	 ...
	 *  （DESIGN-MEMO §3-1）。起動は `mact_tsk` で行う。 */
...
/*
 *  ========================================================================
 *  音声プール（M5Unified Speaker/Mic）の生成（2026-08-05・段7）
 *  ========================================================================
 ...
 *
 *  **失敗したら `del_tsk` で巻き戻す**（DESIGN-MEMO §3-2(c)。リーク防止）。
...
esp_shim_tsk_activate(ID tskid, uint32_t core_id)
{
	...
	{
		/*
		 *  BL-B-1 / LM-N-3（2026-08-07）: これまでの判定 STALE の根拠は
