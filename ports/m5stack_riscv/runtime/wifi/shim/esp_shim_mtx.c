 ...
 *
 *  ========================================================================
 *  何が変わったか（2026-08-04・段1）
 *  ========================================================================
 *  従来は `esp_shim.cfg` の `CRE_MTX` ×20（構成により 8/12/16 本が有効）を
 *  静的プールとして持ち、`esp_shim.c` の `shim_mtx[]` が空きスロットを線形
 *  探索して配っていた。**本 TU はそのプール管理を廃し、カーネルの動的生成
 *  サービスコール `acre_mtx` / `del_mtx` を直接使う。**
 *
 *  **静的版と動的版の分岐は無い**（`#ifdef` も実行時分岐も）。
 *    無条件に動的生成である。理由は `1765759`（キュー写像先の分岐を廃止）
 *    と、その分岐が実際に H4 を再発させた `.steering/20260803-h4-stale-ar-hypothesis/`。
 *
 *  **ミューテックスはカーネルメモリプール（`DEF_MPK`）を使わない**
 *    （`fmp3/fmp3_core/docs/dynamic-creation.md` §3.3「`acre_*`：`E_NOID` のみ
 *     （メモリプール不使用）」）。⇒ bump アロケータが単調に痩せる問題
 *    （`.steering/20260804-dynamic-object-integration/DESIGN-MEMO.md` §4-4b）とは無関係で、
 *    `DEF_MPK` を足す必要も無い。
 *
 *  ========================================================================
 *  ハンドルの表現 — **ID を裸で返してはいけない**（実測に基づく）
 *  ========================================================================
 ...
 *      （＝ 2026-07-17 に見つかった「配列が 8 個しか初期化されず 9 個目以降が
 *        エイリアスする」型の欠陥が**構造的に作れない**。
 *        `.steering/20260717-shim-pool-gap-verification/`）
 ...
 *  でしか成功しない。旧実装は `SHIM_LOCK`（`rsil 15`）の中でスロットを配って
 *  いたので**どの文脈でも黙って通っていた**。
 ...
 *  旧実装は死んだハンドルを黙って受け取り、`loc_mtx(0)` が必ず失敗する
 *    ＝**排他が黙って消える**形だった（同上 2026-07-17 の記録）。
 */
static SHIM_MTX *
