...
#ifdef ESP32C6_USJ_PROBE
/*
 *  段3 Task 1: USJ 1 文字落ちの計測用カウンタ（計測専用、ESP32C6_USJ_PROBE
 *  未定義なら 1 バイトも変わらない）。
 ...
 *        BUS_RESET 再武装が実際に発火した回数。A1_C6_APPDIR=test_ovr の
 *        ように serial.cfg が無く ISR（INTNO_SIO）が有効化されない構成
 *        （ena_int が E_OBJ を返す）でも、この経路だけは動く
 */
volatile uint32_t	esp32c6_usj_cnt[6];
 ...
 *  （esp32c6_usbjtag_isr_siop の BUS_RESET 分岐のコメント参照）。
 *  この再武装ロジックは ISR（esp32c6_usbjtag_isr_siop）と，ISR が
 *  一度も動かない構成（A1_C6_APPDIR=test_ovr のように serial.cfg が
 *  無く ena_int(INTNO_SIO) が E_OBJ を返し INTNO_SIO が有効化されない
 *  場合）向けの低レベル出力側ポーリング（esp32c6_usbjtag_snd_chr）の
 ...
 */
static void
esp32c6_usbjtag_in_token_watch_arm(void)
{
...
static void
esp32c6_usbjtag_bus_reset_rearm(void)
{
	usb_serial_jtag_ll_clr_intsts_mask(USB_SERIAL_JTAG_INTR_BUS_RESET);
	if (usb_serial_jtag_ll_txfifo_writable() == 0) {
		usb_serial_jtag_ll_txfifo_flush();
		esp32c6_usbjtag_in_token_watch_arm();
	}
}
...
esp32c6_usbjtag_snd_chr(SIOPCB *p_siopcb, char c)
{
	...
	/*
	 *  段4 fix wave 1 (G): ISR（esp32c6_usbjtag_isr_siop）が動かない構成
	 *  （A1_C6_APPDIR=test_ovr。上の esp32c6_usbjtag_bus_reset_rearm の
	 *  コメント参照）では、open 時の1回きりの再武装（opn_por）だけでは
	 *  opn_por より後に起きたバスリセットを取りこぼす。ここは
	 *  esp32c6_sio_fput（chip_serial.c）のリトライループから毎回呼ばれる
	 ...
	 *  ISR だけでなく、esp32c6_sio_fput（target_fput_log のポーリング出力）から
	 *  タスク文脈・割込み許可のまま呼ばれる。IN トークン監視の武装
	 *  （esp32c6_usbjtag_in_token_watch_arm）は INT_ENA の read-modify-write で、
	 *  ISR 側（esp32c6_usbjtag_isr_siop）が同じレジスタを disable_intr_mask で
	...
	if ((usb_serial_jtag_ll_get_intsts_mask() & USB_SERIAL_JTAG_INTR_BUS_RESET) != 0U) {
		SIL_LOC_INT();
		esp32c6_usbjtag_bus_reset_rearm();
		SIL_UNL_INT();
#ifdef ESP32C6_USJ_PROBE
		esp32c6_usj_cnt[5]++;
#endif /* ESP32C6_USJ_PROBE */
...
esp32c6_usbjtag_isr_siop(SIOPCB *p_siopcb)
{
	...
	if ((stat & USB_SERIAL_JTAG_INTR_TOKEN_REC_IN_EP1) != 0U) {
		/*
		 *  host が EP1 へ IN トークンを送ってきた（esp32c6_usbjtag_in_token_watch_arm
	...
	if ((stat & USB_SERIAL_JTAG_INTR_SERIAL_IN_EMPTY) != 0U) {
		 ...
		 *  が来る）。ESP32C6_USJ_NOFIX（既定 OFF）は修正前の無条件呼出しを
		 *  positive control 用に復元するスイッチ（Step 5、AC-1c）。
		 */
#ifdef ESP32C6_USJ_PROBE
		usj_in_irdy = true;
#endif /* ESP32C6_USJ_PROBE */
#ifdef ESP32C6_USJ_NOFIX
		esp32c6_usbjtag_irdy_snd(p_siopcb->exinf);
#else /* ESP32C6_USJ_NOFIX */
		if (usb_serial_jtag_ll_txfifo_writable() != 0) {
			esp32c6_usbjtag_irdy_snd(p_siopcb->exinf);
		}
#endif /* ESP32C6_USJ_NOFIX */
#ifdef ESP32C6_USJ_PROBE
		usj_in_irdy = false;
#endif /* ESP32C6_USJ_PROBE */
	}
