 ...
 *  （M5NanoC6 / ESP32-C6 用・FMP3・段4 Task 3 minor round、2026-09-14）
 *
 *  target_timer.h の target_hrt_get_current64() マクロの実体。呼び手は
 *  esp/shim/esp_shim.c の esp_shim_time_us()（タスク文脈）。
 *  diag_recorder.c は 32bit の esp32c6_systimer_read() を MIE=0 で使い、
 *  こちらは呼ばない（fix wave 2 でコメント訂正）。カーネル用の
 *  esp32c6_systimer_read()（target_timer.h。CPU ロック下で使う）と違い、
 ...
 *  リンクは cmake/a1_c6_stage1.cmake の A1_C6_WIFI ブロックだけ
...
int64_t
esp32c6_systimer_read64_us(void)
{
	return((int64_t)(esp32c6_systimer_read64() / ESP32C6_SYSTIMER_TICKS_PER_US));
}
