...
/*
 *  タイマドライバのターゲット依存部（M5NanoC6 / ESP32-C6 用・FMP3・esp-hal LL層版）
 *
 *  出典: asp3_esp_idf asp3/target/esp32c6_espidf/target_timer.h。単一コアの
 ...
 *  （esp32c6_hrt_set_event_*・g_hrt_last_target）は落とした。
 *    - HRTCNT（us）はカウント値の1/16（較正済み．esp32c6.h参照．
 ...
 *  タイマ割込みハンドラ登録のための定数
 */
#define INTNO_TIMER        16                         /* 割込み番号（線16。線1-15 は Wi-Fi shim 予約） */
...
target_hrt_set_event(ID prcid, HRTCNT hrtcnt)
{
	...
	(void) prcid;
	current = esp32c6_systimer_read();
	target = current + (uint64_t)hrtcnt * ESP32C6_SYSTIMER_TICKS_PER_US;
 ...
 *  esp32c6_systimer_read() を使う。2026-09-15 訂正）。S3/LX6 は CCOUNT の 32bit ラップを
 *  64bit 累積器で補う実装だが、C6 の SYSTIMER は 52bit・16MHz のフリーラン
 *  カウンタなので us へ割るだけでよい（2^52 / 16MHz = 8.9 年でラップ）。
 *
 *  実体は target_hrt64.c（esp32c6_systimer_read64()）。上の
 *  esp32c6_systimer_read() は hi -> lo を 1 回ずつ読むだけで、カーネル内の
 *  呼び手（CPU ロック下）では十分だが、本マクロの呼び手はタスク文脈・Wi-Fi ISR
 *  からで、hi と lo の間で割り込まれて別の snapshot が挟まると lo のラップ
 *  （268 秒ごと）を跨いで 2^32 ずれた値になる。esp32c6_systimer_read64() は
 ...
 *  戻してよい。target_hrt64.c は cmake の A1_C6_WIFI ブロックだけがリンクする。
 */
extern int64_t esp32c6_systimer_read64_us(void);
#define target_hrt_get_current64()	esp32c6_systimer_read64_us()
