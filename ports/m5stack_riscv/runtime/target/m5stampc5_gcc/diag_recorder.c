...
/*
 *  クラッシュ/ハング診断用 recorder（M5Stamp-C5 / ESP32-C5 用・FMP3）の縮小実装
 *
 ...
 *  （16MHz tick）。リンクは cmake/a1_c5_stage1.cmake（段4 で足す） の A1_C5_WIFI ブロック
...
diag_ts(void)
{
	return((uint32_t) esp32c5_systimer_read());
}
...
diag_boot_check_and_dump(intptr_t exinf)
{
	...
	(void) exinf;
	syslog(LOG_NOTICE, "diag(c5): RAM ring only, no cross-reset persistence");
}
