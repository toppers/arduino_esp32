/*
 *  vendored sdkconfig.h — ESP32-S3(LX7) FMP3ビルド用 CONFIG_* 定義。
 *  出自: esp-hal-3rdparty (asp3_esp_idf/hal) の NuttX port 生成 sdkconfig.h
 *  ($HAL/nuttx/esp32s3/include/sdkconfig.h) をリポジトリへ取り込み、
 *  esp-hal-3rdparty($HAL) への参照を断つ（ESP-IDF only 方針、S3供給移行 段階3、
 *  2026-07-16）。値は当時の esp-hal 版のまま(バイト等価)。LX6の esp/config/esp32/
 *  sdkconfig.h とはチップ固有のCONFIG_SOC_*が異なるため共用不可、S3専用に分離。
 *  ※ 将来 ESP-IDF Kconfig 生成版へ差し替える場合は menuconfig 相当で再生成する。
 *  下方の <nuttx/config.h> は TOPPERS 自身の asp3 hal_stub
 *  （esp/config/esp32/hal_stub_include、チップ非依存で共用）から解決される。
 */
/* The following section was automatically generated from ESP-IDF's softap
 * example for ESP32-S3 (examples/esp/getting_started/softAP). Please,
