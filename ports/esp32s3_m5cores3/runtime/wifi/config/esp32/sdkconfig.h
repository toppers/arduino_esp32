/*
 *  vendored sdkconfig.h — 無印ESP32(LX6) FMP3ビルド用 CONFIG_* 定義。
 *  出自: esp-hal-3rdparty (asp3_esp_idf/hal) の NuttX port 生成 sdkconfig.h を
 *  リポジトリへ取り込み、esp-hal-3rdparty($HAL) への参照を断つ（ESP-IDF only 方針、
 *  段階4 config-plumbing 撤去、2026-07-16）。値は当時の esp-hal 版のまま（バイト等価）。
 *  ※ 将来 ESP-IDF Kconfig 生成版へ差し替える場合は menuconfig 相当で再生成する
 *    （段階3 mbedtls config と連動）。line 293 の <nuttx/config.h> は TOPPERS 自身の
 *    asp3 hal_stub（$IDF/asp3/.../hal_stub/include）から解決される。
 */
/*
