...
#endif
    syslog(LOG_NOTICE, "DIAG pre-internal config=%x", (int)(uintptr_t)config);
    result = esp_wifi_init_internal(config);
    syslog(LOG_NOTICE, "DIAG post-internal result=%x", (int)result);
    if (result == ESP_OK) {
#if CONFIG_MAC_BB_PD
...
int wifi_nan_set_config_local(void *p)
{
    /* Do not remove, stub to overwrite weak link in Wi-Fi Lib */
    return 0;
}
