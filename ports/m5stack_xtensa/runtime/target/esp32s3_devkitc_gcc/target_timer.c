...
/*
 * TTSP3のgain_tick（凍結中に1 tick分だけ強制的に
...
/*
 * target_hrt_get_current()のソフトウェア64bit拡張用
 * per-core状態（宣言・詳細コメントはtarget_timer.h参照。
 * 追記51/52）。
 *
 * ★2026-07-17改訂（Bug A の修正＝F1）：
 *   旧コメントはここに「BSS上でゼロ初期化されるため，明示的な初期化コードは
 *   不要（…従来どおり『コアリセットからの経過』を表す値になる）」と書いて
 *   いたが、これは**設計上の欠陥**だった。「コアリセットからの経過」は
 *   **コアごとに時刻原点が違う**ことを意味する。ESP32-S3のCPU1はチップ
 *   リセットから大幅に遅れて（seam起動の実測で約150ms後）リセット解除
 *   されるため、CPU0とCPU1のCCOUNTには**定数オフセットΔ**が乗る。
 *
 *   一方FMP3カーネルは current_evttim / current_hrtcnt を**全プロセッサ
 *   共有のグローバルスカラ**として持ち（kernel/time_event.c）、
 *   update_current_evttim() が
 *       hrtcnt_advance = target_hrt_get_current() - current_hrtcnt
 *   というグローバル差分演算を行う。すなわち target_hrt_get_current() は
 *   「**どのコアで呼んでも同じ絶対時刻を返す単一の時刻源**」であることを
 *   要求する（他ポートはいずれもコア間共有のハードタイマを使う：
 *   arch/arm_gcc/common/mpcore_timer.h=MPCore Global Timer、
 *   arch/riscv_gcc/common/mtimer.h=mtime）。per-coreの原点はこの契約に
 *   違反しており、コア跨ぎのタイムイベント（msta_alm等）で
 *   current_evttim が ±Δ 逆行/早送りする。
 *
 *   実測（JTAG、seam実機 f4:12:fa:5b:4a:58）：
 *       acc[0]=66,063,122 / acc[1]=54,036,376 → Δ=12,026,746 cycles
 *       = 150.33ms実時間。test_malarm1 の再アーム遅延 150/151/151ms と
 *       1ms未満で一致。
 *   
 *
 ...
 *
 * ■なぜ「起動時一度きり」で十分か
 *   ESP32-S3の両コアは同一のCPUクロック（同一PLL/XTAL由来）で駆動される。
 *   CCOUNTはそのクロックで1サイクル1カウント進むだけのカウンタなので、
 *   両コアのCCOUNTの差は**リセット解除時刻の差**そのもの＝**定数**であり、
 *   周波数差に由来するドリフトは原理的に生じない（別クロック源であれば
 *   周期再同期が要るが、そうではない）。よって一度合わせれば恒久的に一致
 *   し続ける。
 *
 * ■実施箇所と起動順序の根拠（実物のブートパスを追って決定）
 *   (1) CPU1は software_init_hook()→target_mprc_initialize()（CPU0のみ、
 *       sta_ker前）で解放され、start.Sのスレーブ経路から sta_ker へ入る。
 *   (2) 両コアは sta_ker（fmp3_trunk/kernel/startup.c）で
 *         barrier_sync(1) → target_initialize → barrier_sync(2)
 *         → initialize_tmevt/initialize_object → barrier_sync(3)
 *         → [master] call_inirtn(global) → barrier_sync(4)
 *         → call_inirtn(自プロセッサのリスト) → barrier_sync(5)
 *         → [TMASTER] current_hrtcnt = target_hrt_get_current()  ←★
 *         → barrier_sync(6) → set_hrt_event → start_dispatch
 *       と進む。
 *   (3) 本関数は target_hrt_initialize() から呼ばれる。target_hrt_initialize は
 *       target_timer.cfg の CLASS(CLS_PRC1)/CLASS(CLS_PRC2) 両方に ATT_INI
 *       されており、**両コアのプロセッサ別inirtnリストの先頭**に来る
 *       （生成物 kernel_cfg.c の _kernel_inirtnb_table_prc1[0] /
 *         _kernel_inirtnb_table_prc2[0] が _kernel_target_hrt_initialize で
 *         あることを実物で確認済み）。
 *       → 両コアは barrier_sync(4) を出た直後に、**間に何も挟まずに**
 *         本関数へ到達する。したがって
 *           ・両コアが確実に生きて到達する（＝ランデブが成立しデッドロック
 *             しない。先行する初期化ルーチンが無いので、片コアが別の
 *             待ちに捕まる経路が存在しない）
 *           ・到達時刻の差が最小（barrier_sync(4)の解放直後）
 *   (4) ★順序上の必須要件：同期は「グローバル時刻がHRT値を最初に取り込む
 *       時点」＝(2)の★印（barrier_sync(5)の直後、startup.c:216）より
 *       **前**でなければならない。本関数は barrier_sync(4)〜(5) の区間で
 *       走るため、この要件を満たす。それ以前に current_hrtcnt は0（BSS）で
 *       あり、ディスパッチもtick割込みも未開始のため、同期による累積器の
 *       書き換えがカーネルのグローバル時刻へ波及することはない。
 *   (5) 本関数は tick(INT6)のenable_intより前に呼ぶ（下の呼出し位置参照）。
 *       ハンドシェイク中にtickが割り込む余地を作らない。
 *
 * ■同期の方式（NTP流の往復中点補償）
 *   CPU1はCPU0のCCOUNTを直接読めないため、共有SRAM経由でランデブする。
 *   単純な「CPU0が公開→CPU1が読む」では公開から読取りまでの遅延が
 *   そのまま誤差になるので、CPU1側で往復時間を測り中点を採る：
 ...
 *   の release/acquire 対で64bit値のtearingを防ぐ（内蔵SRAMはコア間コヒーレント）。
 */
...
target_hrt_sync_origin(void)
{
	...
	if (xtensa_get_prcidx() == 0U) {
		...
		while (hrt_sync_req == 0U) {
			/* CPU1が本関数へ到達するのを待つ（上記(3)によりランデブは保証） */
		}
		Asm("memw" ::: "memory");
...
void target_hrt_initialize(intptr_t exinf)
{
     ...
     *   ランデブ（＝相互待ち合わせ）なので、本関数の先頭で行うことで
     *   「barrier_sync(4)を出た直後・間に何も挟まない」という最小スキューの
     *   位置になる（詳細な根拠は target_hrt_sync_origin() のコメント）。
     */
    target_hrt_sync_origin();
#endif /* TNUM_PRCID >= 2 */

    /*
     * ROMの_UserExceptionVectorはEXCCAUSEでxtos_exc_handler_table
     * を引いて飛ぶ。Level1割込み（EXCCAUSE=4）の
     * スロットへ**自前の割込みエントリ_kernel_l1int_entry**を直接書き込む
     * （割込み復帰時プリエンプションのため。core_support.S、
     * 。
     *
     * arch-bringup時はここへROMの_xtos_l1int_handlerを書いてtickを
     * 発火させていたが、ROMのディスパッチャは復帰が`rfe`に固定配線され
     * ており割込み復帰時のディスパッチを差し込めないため、エントリ自体を
     * 自前化する。_kernel_l1int_entryは中断コンテキストを退避し、
     * _kernel_l1int_dispatch（tick/IPIのdemux。signal_time経由でタイム
     * イベント処理）を割込みスタック上・非タスクコンテキストで実行し、
     * 復帰時にp_runtsk≠p_schedtskなら遅延ディスパッチする。
     * [追記56] 以前は本コメントがtarget_hrt_handler_prc1（実際には
     * 呼ばれないデッドコード）を指していたが誤りだったため訂正。
     *
     * INTENABLEでCCOMPARE0割込み(int6)を有効化する。int6は本プロジェクト
     * 唯一のLevel-1割込み源であり、_kernel_l1int_entryは
     * _kernel_l1int_dispatchを直接呼ぶため、_xtos_set_interrupt_handler
     * によるxtos_interrupt_table登録には依存しない（呼んでも無害）。
     */
    xtos_exc_handler_table[XT_EXCCAUSE_LEVEL1_INTERRUPT] = (_xtos_handler) _kernel_l1int_entry;

    /*
     * Level-3割込みのフック（BT-4調査、.claude/plans/sparkling-forging-taco.md）。
     * ROMの_Level3Vector(0x400001c0)は`xsr.excsave3 a2; jx a2`のみを行い
     * EXCSAVE3レジスタの内容へ間接ジャンプする（ROM ELF実測）。VECBASEを
     * 変更せず、EXCSAVE3へ_kernel_l3int_entry（core_support.S）のアドレスを
     * 書き込むだけでLevel-3割込みをフックできる。EXCSAVE3はコア毎の
     * レジスタのため、本関数はCLS_PRC1/CLS_PRC2の両方（target_timer.cfg）
     * から呼ばれる＝各コアで独立にこの初期化が行われる。
     */
    Asm("wsr.excsave3 %0" :: "a"(_kernel_l3int_entry) : "memory");

    /*
     * tick(INT6)の許可はenable_intで行う（自コアのソフトウェア許可マスク
     * _kernel_intenable_maskとハードのINTENABLEを対で更新する）。ROMの
     * _xtos_ints_onは内部の_xtos_enabledでINTENABLEを上書きし、enable_intや
     * CFG_INTで許可した割込み（例：INT7）のビットを消してしまうため使わない。
     */
...
_kernel_l1int_dispatch(void)
{
    ...
    if ((active & (1U << XT_TIMER_INTNUM)) != 0U) {
        ...
        /*  ★M5 専用（golden 5 構成では未定義＝1 命令も増えない）。
         *  「tick 割込みが来ているのに周期ハンドラだけ動かない」のか
         *  「tick 自体が来ていない」のかを弁別するための唯一の直接証拠。 */
        _m5_tdiag[M5_TDIAG_TICK]++;
    ...
    /*
         *  ★IPI 受信経路は「active にセットされているビットだけを昇順に処理」する。
     *  「割込み番号0〜31の全ビット位置を無条件に走査」してはならない。
     *  呼ばれるハンドラの集合・順序・回数は完全に同一で（下記の等価性1〜4）、
     *  変わるのは「空振りする反復を踏まない」ことだけである。
     *
     *  ■なぜそれが要るか
     *  この走査は IPI 受信経路の**tail**（＝FROM_CPU トリガをクリアしてから
     *  rfe で中断タスクへ戻るまでの区間）の中にあり、tail の最大の構成要素
     *  だった。前進の必要条件は tail < gap（gap＝送出側が次にトリガを立てる
     *  までの沈黙時間）である。rfe の瞬間に次の IPI が既にペンディングして
     *  いると、中断タスクは1命令も retire できずに再捕捉される
     *  （＝古典的な receive livelock）。
     *
     *  ★★**限界（将来の担当者へ）**：これは tail を予算内に収める
     *  「余裕の改善」であって**「前進保証」ではない**。人工的な位相掃引では
     *  通らない点が残る。真の前進保証は「IPI 処理後に INT13 を一時的に
     *  INTENABLE から落とし、victim を N cycle 走らせてから再許可」＝
     *  割込み駆動→ポーリング切替であり、別課題として残っている。
     *
     ...
     *      触ることはない。
     *
     *  ESP32-S3（Xtensa LX7）はNSAU命令を持つため__builtin_clzは1命令に
     *  展開される（ライブラリ呼出しにならないことを逆アセンブルで確認済み）。
 */
    while (active != 0U) {
