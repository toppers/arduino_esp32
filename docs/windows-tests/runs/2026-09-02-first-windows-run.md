...
**方針を「BOM 無しの `.ps1` には ASCII しか書かない」に決めて、揃えた。**
`New-Fmp3PrebuiltStages.ps1` の `★` を落とし、`scripts/*.ps1` 23 本すべてが
BOM 無し・非 ASCII ゼロになった。`Build-SeamS3M5.ps1` は今回の書き換えで
日本語コメントごと無くなったので BOM も要らなくなっている。
この run で足したコメントも全部英語にして、周囲のコメントの言語に合わせた。

コードページ依存が原理的に無くなるので、BOM を付けて回るより確実。判定は
「非 ASCII があって BOM が無い `.ps1` はゼロか」の 1 行で済む。
