...
**追記（2026-09-12）: このテストは、組み立てた platform ではなく Boards
Manager 版でビルドしていた。**

隔離したのは sketchbook だけで、`data:` は実機の `Arduino15` を指していた。
...
（同じ run が Platform: ...build\stage-platform\sketchbook\... と表示）
```

**影響範囲は入れ替わりが起きていた期間だけ**で、思ったより狭い。この機械の
`Arduino15` に `toppers` が入ったのは **2026-09-11 14:28（0.4.1）**で、
ダウンロードキャッシュにも 0.3.0 のアーカイブは無い。つまり
`runs/2026-09-02-first-windows-run.md` と、そこから B-1〜B-4 で引いている
**9 ビルドの結果は影響を受けていない**（当時 `toppers` は入っていなかった）。

**影響を受けたのは 1 件だけ**である。上の 2026-09-05 の追記が引いている
M5StickS3 の `m5` 行の検証は、0.4.1 に対して建てたものだった。0.4.1 にも
その板と構成は入っているのでビルド自体は本物だが、**ツリーのステージと
`install_platform.py` の出力を試したことにはなっていない。**

直し方は 2 つで、両方入れた。

1. **`toppers` だけを含まないデータディレクトリを組み立てる。** 他のパッケージは
   junction で張るので容量も要らず、昇格も要らない。
2. **解決先が組み立てた platform であることを毎回検査する。** 判別は名前で行う
   ——`install_platform.py` が書く `M5Stack Arduino with TOPPERS/FMP3` と、
   Boards Manager 版が持つ index 由来の `TOPPERS/FMP3 M5Stack boards` は
   別の文字列である（実測で区別できることを確認した）。

**この隔離ディレクトリは短いパスに置くこと。** 最初リポジトリ内の
`build\stage-platformrduino-data` に作ったところ、M5Core のビルドだけが
C++ 標準ヘッダを見失って落ちた。コンパイラが見るツールチェーンのパスが
すべてこの根の下に組み直され、GCC が自分のヘッダをたどる
`bin/../lib/gcc/<target>/14.2.0/../../../../<target>/include/c++/...` という
長い相対経路が 260 文字を超えるためである。**同一のビルドが、短い根では成功し、
深い根では失敗した**（コンパイル行の差はパスの根だけ）。`BUILDING.md` の
「5. リリースする」に同じ罠が利用者側の形で書いてある。

### この 2 つを隠していたもの: arduino-cli のコアキャッシュ

どちらも**すぐには出なかった**。arduino-cli は M5Stack コアのコンパイル結果を
全体で共有するキャッシュに置き、その鍵にはパスを含むビルドプロパティが入る。
鍵が一致する限りコアは再コンパイルされないので、

- **入れ替わり**は、両方の platform が同じキャッシュを引く限り同じ結果を出す
- **260 文字の問題**は、コアを実際にコンパイルするときにしか現れない

という形で伏せられていた。切り分けの最中、成功していた比較対象が
`Esp.cpp` を一度もコンパイルしていないことに気づくまで、私はこれを
「LX6 のツールチェーン同梱の欠落」と誤診しかけた。**比較実験をするときは、
コアが実際にコンパイルされたかを確かめること**（`--verbose` の出力に
`Esp.cpp` が出るかで分かる）。

なお PowerShell の落とし穴も 1 つ踏んだ。`$ErrorActionPreference = 'Stop'` の
下では、native コマンドの stderr は**呼び出し側が出力をまとめた瞬間に**終了
エラーになる。arduino-cli は低メモリ警告を stderr に出すので、全 10 ビルドの
バイナリが揃ったあとの 9 本目で run が死んだ。判定は終了コードで行っているので、
compile の呼び出しの間だけ `Continue` に戻すようにした。
