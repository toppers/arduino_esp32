#!/usr/bin/env bash
#
#  scripts/capture_c5_usj.sh -- gated flash-and-capture for M5Stamp-C5 (ESP32-C5)
#  ===========================================================================
#
#  Writes the four images an `arduino-cli compile` run produces for the
#  toppers:esp32:m5stampc5_fmp3 board to the M5Stamp-C5 over its USB Serial/JTAG
...
#
#  This is a copy of scripts/capture_c6_usj.sh (the M5NanoC6 script; that
#  file is untouched) with the ESP32-C5 / M5Stamp-C5 specifics of the
#  development-side esp/boot/flash_and_capture_c5_usj.sh (fmp3_esp_idf_dev)
#  folded in. What differs from the C6 script:
#    - the DUT is the M5Stamp-C5 (BASE MAC 3c:dc:75:8d:ed:20, chip string
#      "ESP32-C5 (revision v1.0)", 4MB) and the FORBIDDEN list additionally
#      holds the M5NanoC6 (9c:13:9e:d3:62:18): this script must never write
#      to the C6 board that sits on the neighbouring hub port.
#    - the bootloader address is NOT a literal: it is read from the installed
#      platform's boards.txt (BOARDS_TXT, key m5stampc5_fmp3.build.bootloader_addr)
#      and MUST be 0x2000 (the ESP32-C5 ROM loads the 2nd-stage bootloader
#      from 0x2000; the C6 uses 0x0). Any other value, or a missing key, is a
#      gate failure. Partition table 0x8000 / boot_app0 0xe000 / app 0x10000
#      are the M5Stack upload recipe's, as on the C6.
#    - EVERY writing run first erases flash 0x0-0x1FFF (esptool erase-region
#      0x0 0x2000) and, after the write, reads that range back and requires
#      it to be all 0xFF. Reason: this board came with an asp3 Direct Boot
#      image whose magic (0xaedb041d x2) still sits at flash 0x0; the ROM
#      prefers Direct Boot over the 0x2000 bootloader whenever that magic is
#      present, so a write that leaves 0x0 alone boots the OLD image. The
#      erase is idempotent (two 4 KB sectors) and is shown in DRYRUN. The
#      factory backup of the whole flash lives on the development side
#      (build/c5-backup/m5stampc5-factory-3CDC.758D.ED20.bin); this script
#      never restores it.
#    - the esptool calls of one writing run are chained in the ROM download
#      mode (flash-id --after no-reset, then erase-region / write-flash with
#      --before no-reset --after no-reset, then read-flash --before no-reset
#      --after hard-reset), the sequence the development-side script proved
#      on this board (esptool 5.2.0 re-uploads its stub per call, no reset in
#      between).
#    - the JTAG liveness probe uses board/esp32c5-builtin.cfg and, besides
#      the USB_SERIAL_JTAG registers (same addresses on the C5), reads the
#      PCR clock registers PCR_SYSCLK_CONF (0x60096110) and PCR_CPU_FREQ_CONF
#      (0x60096118) plus the runtime's g_seam_c5_clk_result (8 words in
#      .data) -- the CPU-frequency evidence for stage 2 (the minimal stage
#      prints no PCR line). C5_JTAG_FORCE=1 on a healthy warm run collects it.
...
#      images so one axis at a time can be swapped (stage 2 arms B/C).
#    - esptool is the M5Stack core's bundled esptool_py (5.x, dash-style
#      subcommands: write-flash, flash-id, erase-region, read-flash). The
#      write-flash arguments are the upload recipe's (-z, --flash-mode/
#      --flash-freq/--flash-size keep).
...
#    2b. BOARDS_TXT exists, holds m5stampc5_fmp3.build.bootloader_addr, and
#       the value is 0x2000.
...
#  After a write: read-flash 0x0 0x2000 must be all 0xFF, and write-flash
#  must have printed "Hash of data verified" once per image; otherwise the
#  run fails and .sha.txt says the flash contents are suspect.
#
#  Usage:
#    SKETCH_BUILD=<dir> bash scripts/capture_c5_usj.sh
#
#  Inputs (environment):
...
#                  The key m5stampc5_fmp3.build.bootloader_addr must exist
#                  and be 0x2000; there is no override variable on purpose.
...
#    DUT_MAC       expected BASE MAC (default 3c:dc:75:8d:ed:20, the M5Stamp-C5).
#    DUT_CHIP      expected chip type string (default "ESP32-C5 (revision v1.0)",
#                  the full string including the revision, as the
#                  development-side script checks it; "ESP32-C5" alone would
#                  also accept another revision or a C5 variant).
#    DUT_PORT      serial device. Default: /dev/serial/by-id name derived
#                  from DUT_MAC (Espressif USB JTAG/serial debug unit).
#    OUT           capture log path (default LOG_DIR/c5-capture-<stamp>.log).
#                  Sidecars are written next to it: .ident.log (flash-id),
#                  .flash.log (erase-region + write-flash + read-flash),
...
#                  summary; only when the probe ran, see C5_JTAG_ON_SILENT).
#    LOG_DIR       default directory for OUT
#                  (default $HOME/TOPPERS/ESP32/fmp3_esp_idf_dev/.steering/
#                   20260916-c5-arduino-plan/stage2/logs).
...
#                  e.g. EXTRA_MARKERS='[C5-INTR] VERDICT PASS|[C5-RGB] tx_done'
#                  (the brackets are literal, which is why it is -F).
#    DRYRUN=1      identify the DUT read-only, print what WOULD be done (the
#                  erase-region, the four writes with sha256 and address, the
#                  read-back) and the exact esptool command lines, write
...
#                  opens the port.
#    C5_JTAG_ON_SILENT  (default 1) after the marker count, when COLD=1 AND
#                  heartbeat=0 (a silent cold run: no output at all), run the
#                  JTAG liveness probe (below). 0 disables it.
#    C5_JTAG_FORCE (default 0) 1 runs the probe regardless of the counts --
...
#    C5_MASK_SELFTEST=1  run the redact/mask self-test, the marker-count
...
#    C5_REDACT_ONLY=1  `C5_REDACT_ONLY=1 bash capture_c5_usj.sh <file>...`
...
#                  C5_REDACT_ANYWHERE=1 lifts the LOG_DIR bound (never the
...
#  (c5_jtag_wanted: DRYRUN never; C5_JTAG_FORCE=1 always; else COLD=1 and
#  heartbeat=0 with C5_JTAG_ON_SILENT=1), the script runs, under a 30 s
#  timeout, capturing stdout+stderr raw to .jtag.log:
#    openocd -s <scripts> -c "adapter serial <DUT_MAC upper-case>" \
#            -f board/esp32c5-builtin.cfg -c init -c halt -c "reg pc" \
...
#  no value is trusted, and a [C5] warning is printed. When openocd prints
...
#  usb_serial_jtag_reg.h, esp32c5). EP1_CONF and INT_RAW are RECORDED RAW
...
#    smark      a line that is just "S" (SEAM_C5_ENTRY_MARK, printed by the
...
#    apm        "wifi_adapter(c5): apm " readback lines (before-unblock /
...
#  ---------------------------------------------------------------- DUT identity
#  Exactly one C5 board exists (M5Stamp-C5). A typo here would write to somebody
#  else's board, so the default is a literal and the FORBIDDEN list is the
#  development-side one (the ESP32-P4 boards plus the asp3 C6 devkit) PLUS
#  the M5NanoC6 (9c:13:9e:d3:62:18, hub 2-3.3 port 3, next to this board's
#  port 4): the C6 script's DUT is this script's forbidden board.
DUT_MAC="${DUT_MAC:-3c:dc:75:8d:ed:20}"
DUT_CHIP="${DUT_CHIP:-ESP32-C5 (revision v1.0)}"
...
#  boards.txt), key BOARDS_KEY, and required to be exactly 0x2000 (the
#  ESP32-C5 ROM's 2nd-stage bootloader offset; the C6 script's literal 0x0
#  would put the bootloader where the ROM never looks). Prints the value on
...
c5_bootloader_addr() {   # <boards.txt> -> "0x2000" or a reason (rc 1)
    ...
    if [ "$((v))" -ne "$((C5_BL_ADDR_EXPECTED))" ]; then
        echo "key $BOARDS_KEY is $v in $f, expected $C5_BL_ADDR_EXPECTED (the ESP32-C5 bootloader offset); refusing"
        return 1
    fi
    printf '0x%x\n' "$((v))"
}
...
#  C5_MASK_SELFTEST=1. The checker's silence is never taken as success: if a
...
c5_redact_transform() {
    ...
    sed -i -E \
        -e "s/${eui:-__none__}/__C5_DUT_EUI_LC__/g" -e "s/${euiU:-__none__}/__C5_DUT_EUI_UC__/g" \
        -e "s/${lc:-__none__}/__C5_DUT_MAC_LC__/g"  -e "s/${lcU:-__none__}/__C5_DUT_MAC_UC__/g" \
        ...
        -e "s/__C5_DUT_EUI_LC__/${eui:-}/g" -e "s/__C5_DUT_EUI_UC__/${euiU:-}/g" \
        -e "s/__C5_DUT_MAC_LC__/${lc:-}/g"  -e "s/__C5_DUT_MAC_UC__/${lcU:-}/g" "$f" || return 1
    return 0
}
...
_c5_peer_re='[0-9a-f][0-9a-f](:[0-9a-f][0-9a-f]){5}|ta(hi|lo):0x[0-9a-f]|\b[0-9]{1,3}(\.[0-9]{1,3}){3}\b|address=0x[0-9a-f]{8}'
c5_residue_count() {
    local f="$1" lc eui out i needle n=0 found
    lc="$(_dut_forms_lc | sed -n 1p)"; eui="$(_dut_forms_lc | sed -n 2p)"
    out="$( set -o pipefail
            tr 'A-Z' 'a-z' < "$f" | sed -e "s/${eui:-__none__}//g" -e "s/${lc:-__none__}//g" \
              | { $GREP -acE "$_c5_peer_re" || [ $? -eq 1 ]; } )" || return 1
...
c5_redact_on_exit() {
    ...
    for f in ${C5_FILES[@]+"${C5_FILES[@]}"}; do
        [ -f "$f" ] || continue
        n=$((n + 1))
        c5_redact_file "$f" || bad=1
    done
    if [ "$n" -eq 0 ]; then
        echo "[C5] redact: nothing was written by this run; nothing to redact" >&2
    elif [ "$bad" -eq 0 ]; then
        echo "[C5] redact: $n file(s) masked and checked clean" >&2
    fi
    if [ "$bad" -ne 0 ]; then
        [ "$rc0" -ne 0 ] && exit "$rc0"
        exit 93
...
#  Counts only; verdicts are the caller's. Exercised by C5_MASK_SELFTEST=1.
C5_UNEXPECTED_RE='## Unexpected|## Assertion|## Internal|Unregistered (exception|interrupt)|mcause ?=|\[LWIP-ASSERT\]|abort\(\) called|ROM newlib abort\(\)|libc: _(exit|kill)\('
c5_count_markers() {
    local f="$1"
    ...
    n_apm="$(_cnt 'wifi_adapter\(c5\): apm ')"
...
c5_apm_lines() { $GREP -aE 'wifi_adapter\(c5\): apm ' "$1" || true; }

#  ---------------------------------------------------------------- extra markers
#  EXTRA_MARKERS: `|`-separated FIXED strings, each counted with grep -F
#  ("[C5-INTR] VERDICT PASS" is a literal, not a bracket expression). Prints
#    extra: <string1>=<n> <string2>=<n> ...
#  or nothing when EXTRA_MARKERS is empty. Exercised by C5_MASK_SELFTEST=1.
...
#    c5_jtag_wanted <cold> <heartbeat> <force> <dryrun> [<on_silent>=1]
#  prints yes/no. DRYRUN wins over everything; then C5_JTAG_FORCE; then the
#  silent-cold rule (COLD=1, heartbeat=0, C5_JTAG_ON_SILENT=1).
c5_jtag_wanted() {
...
#    c5_jtag_parse <raw openocd log> <loop_calls addr hex or ""> [<why unavailable>] [<openocd rc>]
...
#  PCR register addresses (esp-idf components/soc/esp32c5/register/soc/
#  pcr_reg.h: DR_REG_PCR_BASE 0x60096000, SYSCLK_CONF +0x110, CPU_FREQ_CONF
#  +0x118). SOC_CLK_SEL = bits [17:16] of SYSCLK_CONF (3 = PLL_F240M);
#  CPU_DIV_NUM = bits [7:0] of CPU_FREQ_CONF, divider = CPU_DIV_NUM + 1
#  (hal/esp32c5/include/hal/clk_tree_ll.h). The 2nd-stage bootloader hands
#  over with SEL=3, DIV_NUM=2 (80 MHz); the 240 MHz runtime writes DIV_NUM=0.
C5_PCR_SYSCLK_CONF=0x60096110
C5_PCR_CPU_FREQ_CONF=0x60096118
#  Decodes the two PCR words into "pcr: ... cpu=<MHz|?>" (pure; selftested).
c5_pcr_decode() {   # <sysclk_conf hex8> <cpu_freq_conf hex8>
    local sys="$1" cpu="$2" sel div xtal mhz src
    sel=$(( (16#$sys >> 16) & 3 )); xtal=$(( (16#$sys >> 24) & 0x7f )); div=$(( (16#$cpu) & 0xff ))
    case "$sel" in
        0) src=XTAL; mhz=$(( xtal / (div + 1) )) ;;
        1) src=RC_FAST; mhz="?" ;;
        2) src=PLL_F160M; mhz=$(( 160 / (div + 1) )) ;;
        3) src=PLL_F240M; mhz=$(( 240 / (div + 1) )) ;;
    esac
    echo "pcr: sysclk_conf=0x$sys cpu_freq_conf=0x$cpu soc_clk_sel=$sel ($src) xtal=$xtal cpu_div_num=$div cpu=${mhz}MHz"
}
c5_jtag_parse() {
    local raw="$1" addr="${2:-}" why="${3:-no ELF}" orc="${4:-}" clk="${5:-}"
    local pc1 pc2 ep1 intraw a b free delta n sysc cpuc line
    if [ ! -s "$raw" ]; then
    ...
    #  Clock evidence (never part of the verdict). openocd prints the mdw
    #  address in lower case with 8 digits.
    sysc="$(_nth_hex 1 "$(printf '0x%08x' "$((C5_PCR_SYSCLK_CONF))"): [0-9a-f]{8}")"
    cpuc="$(_nth_hex 1 "$(printf '0x%08x' "$((C5_PCR_CPU_FREQ_CONF))"): [0-9a-f]{8}")"
    if [ -n "$sysc" ] && [ -n "$cpuc" ]; then
        c5_pcr_decode "$sysc" "$cpuc"
    else
        echo "pcr: not read (sysclk_conf=${sysc:-missing} cpu_freq_conf=${cpuc:-missing})"
    fi
    if [ -n "$clk" ]; then
        n="$(printf '%08x' "$((16#${clk#0x}))")"
        line="$($GREP -aoiE "0x${n}: ([0-9a-f]{8} ?){8}" "$raw" | head -1 || true)"
        if [ -n "$line" ]; then
            set -- $(printf '%s' "$line" | sed 's/^[^:]*: //')
            echo "clk_result: sysclk_before=0x$1 cpu_freq_before=0x$2 ahb_before=0x$3 sysclk_after=0x$4 cpu_freq_after=0x$5 ahb_after=0x$6 busupd_wait=$((16#$7)) rc=$((16#$8)) ($(case $((16#$8)) in 0) echo NOT_RUN;; 1) echo OK;; 2) echo SKIP_NOT_PLL240;; 3) echo VERIFY_NG;; 4) echo BUSUPD_TIMEOUT;; *) echo unknown;; esac))"
        else
            echo "clk_result: not read (no 8-word line at 0x$n)"
        fi
    else
        echo "clk_result: symbol g_seam_c5_clk_result unavailable"
    fi
    if [ -n "$addr" ]; then
...
c5_jtag_probe() {   # <reason: silent-cold|forced>
    local reason="$1" elf="${ELF:-}" addr="" why="no ELF" nm="" d scripts rc=0 v clk_addr=""
    local -a cmd=()
    ...
    #  The clock evidence: PCR_SYSCLK_CONF / PCR_CPU_FREQ_CONF (the CPU
    #  clock source and divider the runtime left) and g_seam_c5_clk_result
    #  (8 words in .data: before/after readbacks, busupd_wait, rc). Read once,
    #  inside the first halt.
    if [ -n "$nm" ] && [ -x "$nm" ] && [ -n "$elf" ] && [ -f "$elf" ]; then
        clk_addr="$("$nm" "$elf" 2>/dev/null | awk '$3=="g_seam_c5_clk_result"{print $1; exit}' || true)"
        case "$clk_addr" in ''|*[!0-9a-fA-F]*) clk_addr="" ;; esac
    fi
    cmd=("$OPENOCD" -s "$scripts" -c "adapter serial $dut_uc" -f board/esp32c5-builtin.cfg
         -c init -c halt -c 'echo {== read 1 ==}' -c 'reg pc' -c 'mdw 0x6000F004' -c 'mdw 0x6000F008'
         -c "mdw $C5_PCR_SYSCLK_CONF" -c "mdw $C5_PCR_CPU_FREQ_CONF")
    [ -z "$clk_addr" ] || cmd+=(-c "mdw 0x$clk_addr 8")
    ...
    say "  openocd: $OPENOCD"
    say "  elf: ${elf:-none}  loop_calls: $([ -n "$addr" ] && echo "0x$addr" || echo "unavailable ($why)")  g_seam_c5_clk_result: ${clk_addr:+0x}${clk_addr:-unavailable}"
    {
    ...
    c5_jtag_parse "$JTAG_LOG" "$addr" "$why" "$rc" "$clk_addr" >> "$JTAG_TXT"
    {
        echo "# jtag: reason=$reason serial=$dut_uc openocd rc=$rc"
        $GREP -aE '^(verdict|loop_calls|pcr|clk_result):' "$JTAG_TXT" || true
    } >> "$SHA_TXT"
    ...
    { $GREP -aE '^(pc1=|ep1_conf=|pcr|clk_result:|loop_calls:|verdict:)' "$JTAG_TXT" || true; } | sed 's/^/[C5] jtag: /'
    say "jtag: raw -> $JTAG_LOG  parsed -> $JTAG_TXT"
    return 0
}

#  ---------------------------------------------------------------- self-test
if [ "${C5_MASK_SELFTEST:-0}" = "1" ]; then
    ...
    c5_load_needles >/dev/null
    [ "${#NEEDLE_VALUES[@]}" -eq 0 ] || _fail "(0) needles loaded from a missing file"
    _fix_mask
    _pre="$(c5_residue_count "$_st")" || _fail "(1) checker returned non-zero on the fixture"
    [ "$_pre" = "5" ] || _fail "(1) residue before masking is not 5 lines ($_pre)"
    c5_redact_check "$_st" 2>/dev/null && _fail "(2) quarantine branch returned zero"
    ...
    ( C5_FILES=("$_ro/fixture.log"); c5_redact_on_exit 2>/dev/null ) && _rc=0 || _rc=$?
    chmod 755 "$_ro"
    [ "$_rc" -eq 93 ] || _fail "(3) c5_redact_on_exit rc is not 93 ($_rc)"
    ...
    c5_redact_file "$_st" || _fail "(4) residue after transform"
    ...
    _fix_mask
    ( DUT_MAC=""; c5_redact_file "$_st" ) || _fail "(5) transform with DUT_MAC unset failed"
    ...
    _n="$(c5_residue_count "$_sd/missing.log" 2>/dev/null)" && _fail "(6) checker returned zero on a missing file"
    ...
    _pre="$(c5_residue_count "$_st")" || _fail "(7) checker returned non-zero on the needle fixture"
    [ "$_pre" = "5" ] || _fail "(7) residue before redact is not 5 (4 needles + 1 IPv4 line): $_pre"
    c5_redact_file "$_st" || _fail "(8) residue after needle redact"
    ...
    _pre="$(c5_residue_count "$_st")" || _fail "(9) checker failed"
    ...
    #  the checker (1 line), be quarantined by c5_redact_check when NOT
    ...
    _pre="$(c5_residue_count "$_st")" || _fail "(9b) checker failed on the hex-address fixture"
    [ "$_pre" = "1" ] || _fail "(9b) checker does not count an unmasked address=0x<8 hex> as residue ($_pre)"
    c5_redact_check "$_st" 2>/dev/null && _fail "(9b) an unmasked address=0x<8 hex> was not quarantined"
    mv -f -- "$_st.UNREDACTED" "$_st"
    c5_redact_file "$_st" || _fail "(9b) residue after the hex-address transform"
    ...
    printf '%s\n' \
        ...
        'wifi_adapter(c5): apm before-unblock hp_func_ctrl=0x00000001 lp_apm0_func_ctrl=0x00000001 lp_apm_func_ctrl=0x00000001 tee_m4=0x00000000' \
        'wifi_adapter(c5): apm after-unblock hp_apm exception latch: none' \
    ...
    _ml="$(c5_count_markers "$_mk")" || _fail "(10) c5_count_markers returned non-zero"
    ...
    [ "$(c5_apm_lines "$_mk" | wc -l)" -eq 2 ] || _fail "(10) c5_apm_lines did not return 2 lines"
    ...
    _ml="$(c5_count_markers "$_mk")" || _fail "(11) c5_count_markers returned non-zero"
    ...
    : > "$_mk"
    _ml="$(c5_count_markers "$_mk")" || _fail "(12) c5_count_markers returned non-zero on an empty file"
    ...
    #  refused and untouched; (15) the same file with C5_REDACT_ANYWHERE=1,
    ...
    ( C5_MASK_SELFTEST=0 C5_REDACT_ONLY=1 LOG_DIR="$_sd/logs" bash "$_self" "$_self" >/dev/null 2>"$_sd/g13.err" ) && _fail "(13) a git-tracked file was accepted"
    ...
    #  the git-tracked rule is not lifted by C5_REDACT_ANYWHERE
    ( C5_MASK_SELFTEST=0 C5_REDACT_ONLY=1 C5_REDACT_ANYWHERE=1 LOG_DIR="$_sd/logs" bash "$_self" "$_self" >/dev/null 2>&1 ) && _fail "(13b) C5_REDACT_ANYWHERE lifted the git-tracked rule"
    ...
    ( C5_MASK_SELFTEST=0 C5_REDACT_ONLY=1 LOG_DIR="$_sd/logs" WIFI_CREDS="$_sd/no-such-creds.sh" bash "$_self" "$_sd/elsewhere/x.log" >/dev/null 2>"$_sd/g14.err" ) && _fail "(14) a file outside LOG_DIR was accepted"
    ...
    ( C5_MASK_SELFTEST=0 C5_REDACT_ONLY=1 C5_REDACT_ANYWHERE=1 LOG_DIR="$_sd/logs" WIFI_CREDS="$_sd/no-such-creds.sh" bash "$_self" "$_sd/elsewhere/x.log" >/dev/null 2>&1 ) || _fail "(15) C5_REDACT_ANYWHERE=1 did not accept a file outside LOG_DIR"
    [ "$(cat "$_sd/elsewhere/x.log")" = 'got ip <IPv4>' ] || _fail "(15) the file was not masked: $(cat "$_sd/elsewhere/x.log")"
    printf 'got ip 192.168.4.23\n' > "$_sd/logs/y.log"
    ( C5_MASK_SELFTEST=0 C5_REDACT_ONLY=1 LOG_DIR="$_sd/logs" WIFI_CREDS="$_sd/no-such-creds.sh" bash "$_self" "$_sd/logs/y.log" >/dev/null 2>&1 ) || _fail "(15b) a file inside LOG_DIR was refused"
    ...
    _ex="$_sd/extra.log"
    printf '%s\n' '[C5-INTR] VERDICT PASS edges=10' 'noise C5' '[C5-RGB] tx_done=3' \
        '[C5-INTR] VERDICT PASS edges=11' '[C5-INTR] VERDICT FAIL' 'C' '5' '[C5] tag' > "$_ex"
    _el="$(EXTRA_MARKERS='[C5-INTR] VERDICT PASS|[C5-RGB] tx_done' c5_count_extra "$_ex")" || _fail "(16) c5_count_extra returned non-zero"
    [ "$_el" = 'extra: [C5-INTR] VERDICT PASS=2 [C5-RGB] tx_done=1' ] || _fail "(16) extra line: $_el"
    #  regex metacharacters are literal: as an ERE "[C5]" matches every line
    #  holding a C or a 5 (all 8 -- checked here so the discriminator is
    #  known to discriminate); as a fixed string exactly the one "[C5] tag"
    [ "$($GREP -acE '[C5]' "$_ex")" -eq 8 ] || _fail "(16) the ERE control does not match all 8 lines"
    _el="$(EXTRA_MARKERS='[C5]' c5_count_extra "$_ex")" || _fail "(16) c5_count_extra returned non-zero on [C5]"
    [ "$_el" = 'extra: [C5]=1' ] || _fail "(16) '[C5]' was not counted as a fixed string: $_el"
    _el="$(EXTRA_MARKERS='' c5_count_extra "$_ex")" || _fail "(16) c5_count_extra returned non-zero on an empty list"
    [ -z "$_el" ] || _fail "(16) empty EXTRA_MARKERS printed something: $_el"
    _el="$(EXTRA_MARKERS='absent-string' c5_count_extra "$_ex")" || _fail "(16) c5_count_extra returned non-zero on an absent string"
    ...
    _jw() { [ "$(c5_jtag_wanted "$1" "$2" "$3" "$4" ${5:+"$5"})" = "$6" ] || _fail "(17) c5_jtag_wanted $1 $2 $3 $4${5:+ $5} is not $6"; }
    _jw 1 0 0 0 '' yes; _jw 1 5 0 0 '' no; _jw 0 0 0 0 '' no; _jw 0 5 1 0 '' yes; _jw 1 0 0 1 '' no; _jw 1 0 1 1 '' no
    _jw 1 0 0 0 0 no       # C5_JTAG_ON_SILENT=0 disables the silent-cold rule
    ...
    _jfix() {   # <serial line or ""> <pc1> <pc2> <read-1 value> <read-2 value or ""> -> fixture
        {
            ...
            echo 'Info : [esp32c5] Target halted, pc=0x40800abc'
            echo '== read 1 =='
            echo "pc (/32): $2"
    ...
    _jp="$(c5_jtag_parse "$_jl" 0x40800123)" || _fail "(18) c5_jtag_parse returned non-zero"
    ...
    _jp="$(c5_jtag_parse "$_jl" 40800123)" || _fail "(18) c5_jtag_parse (bare addr) returned non-zero"
    ...
    _jp="$(c5_jtag_parse "$_jl" 0x40800123)" || _fail "(18) parse (equal) returned non-zero"
    ...
    _jp="$(c5_jtag_parse "$_jl" 0x40800123)" || _fail "(18) parse (no serial) returned non-zero"
    ...
    _jp="$(c5_jtag_parse "$_jl" 0x40800123 "no ELF" 0)" || _fail "(18) parse (no serial, rc 0) returned non-zero"
    ...
    _jp="$(c5_jtag_parse "$_jl" 0x40800123 "no ELF" 1)" || _fail "(18) parse (no serial, rc 1) returned non-zero"
    ...
    _jp="$(c5_jtag_parse "$_jl" 0x40800123)" || _fail "(18) parse (other serial) returned non-zero"
    printf '%s\n' "$_jp" | $GREP -qxF 'verdict: refused' || _fail "(18) another board's serial is not refused: $_jp"
    _jp="$(c5_jtag_parse "$_jl" 0x40800123 "no ELF" 1)" || _fail "(18) parse (other serial, rc 1) returned non-zero"
    ...
    _jp="$(c5_jtag_parse "$_jl" 0x40800123)" || _fail "(18) parse (backwards) returned non-zero"
    ...
    _jp="$(c5_jtag_parse "$_jl" 0x40800123)" || _fail "(18) parse (second read missing) returned non-zero"
    ...
    _jp="$(c5_jtag_parse "$_jl" "" "no ELF")" || _fail "(18) parse (no symbol) returned non-zero"
    ...
    _jfix "Info : esp_usb_jtag: serial ($dut_uc)" 0x40800abc 0x40800def 000003e8 000007d0
    _jp="$(c5_jtag_parse "$_jl" "")" || _fail "(18) parse (no symbol, pc moved) returned non-zero"
    ...
    : > "$_jl"
    _jp="$(c5_jtag_parse "$_jl" 0x40800123)" || _fail "(18) parse (empty) returned non-zero"
    printf '%s\n' "$_jp" | $GREP -q '^verdict: not-run (no openocd output)' || _fail "(18) empty output is not not-run: $_jp"
    #  --- (19) the bootloader address gate: 0x2000 from boards.txt, everything else refused ---
    _bt="$_sd/boards.txt"
    printf 'm5nanoc6_fmp3.build.bootloader_addr=0x0\nm5stampc5_fmp3.name=X\nm5stampc5_fmp3.build.bootloader_addr=0x2000\n' > "$_bt"
    _ba="$(c5_bootloader_addr "$_bt")" || _fail "(19) 0x2000 was refused: $_ba"
    [ "$_ba" = "0x2000" ] || _fail "(19) value is not 0x2000: $_ba"
    printf 'm5stampc5_fmp3.build.bootloader_addr=0x2000\r\n' > "$_bt"
    _ba="$(c5_bootloader_addr "$_bt")" || _fail "(19) CRLF line was refused: $_ba"
    [ "$_ba" = "0x2000" ] || _fail "(19) CRLF value is not 0x2000: $_ba"
    _bneg() {   # <label> <boards.txt content or MISSING> <reason substring>
        local out
        if [ "$2" = "MISSING" ]; then rm -f "$_bt"; else printf '%b' "$2" > "$_bt"; fi
        out="$(c5_bootloader_addr "$_bt")" && _fail "(19) $1 was accepted: $out"
    ...
    _bneg "empty value" 'm5stampc5_fmp3.build.bootloader_addr=\n' "non-hex"
    _bneg "duplicate key" 'm5stampc5_fmp3.build.bootloader_addr=0x2000\nm5stampc5_fmp3.build.bootloader_addr=0x2000\n' "appears 2 times"
    #  --- (20) PCR decode and the clock-result line in the parser ---
    _pd="$(c5_pcr_decode b0030200 00000002)"
    [ "$_pd" = 'pcr: sysclk_conf=0xb0030200 cpu_freq_conf=0x00000002 soc_clk_sel=3 (PLL_F240M) xtal=48 cpu_div_num=2 cpu=80MHz' ] || _fail "(20) PCR decode (80 MHz): $_pd"
    _pd="$(c5_pcr_decode b0030200 00000000)"
    [ "$_pd" = 'pcr: sysclk_conf=0xb0030200 cpu_freq_conf=0x00000000 soc_clk_sel=3 (PLL_F240M) xtal=48 cpu_div_num=0 cpu=240MHz' ] || _fail "(20) PCR decode (240 MHz): $_pd"
    _pd="$(c5_pcr_decode b0000200 00000001)"
    printf '%s' "$_pd" | $GREP -q 'soc_clk_sel=0 (XTAL) xtal=48 cpu_div_num=1 cpu=24MHz' || _fail "(20) PCR decode (XTAL/2): $_pd"
    _jfix "Info : esp_usb_jtag: serial ($dut_uc)" 0x40800abc 0x40800def 000003e8 000007d0
    _jp="$(c5_jtag_parse "$_jl" 0x40800123 "no ELF" 0 0x40800004)" || _fail "(20) parse with clk returned non-zero"
    printf '%s\n' "$_jp" | $GREP -qxF 'pcr: sysclk_conf=0xb0030200 cpu_freq_conf=0x00000000 soc_clk_sel=3 (PLL_F240M) xtal=48 cpu_div_num=0 cpu=240MHz' || _fail "(20) pcr line: $_jp"
    printf '%s\n' "$_jp" | $GREP -qxF 'clk_result: sysclk_before=0xb0030200 cpu_freq_before=0x00000002 ahb_before=0x00000000 sysclk_after=0xb0030200 cpu_freq_after=0x00000000 ahb_after=0x00000000 busupd_wait=3 rc=1 (OK)' || _fail "(20) clk_result line: $_jp"
    printf '%s\n' "$_jp" | $GREP -qxF 'verdict: alive' || _fail "(20) verdict with clk is not alive: $_jp"
    #  without the symbol the line says so; the verdict is unchanged
    _jp="$(c5_jtag_parse "$_jl" 0x40800123 "no ELF" 0 "")" || _fail "(20) parse without clk returned non-zero"
    printf '%s\n' "$_jp" | $GREP -qxF 'clk_result: symbol g_seam_c5_clk_result unavailable' || _fail "(20) clk_result unavailable line: $_jp"
    #  a wrong symbol address -> not read (no fabricated values)
    _jp="$(c5_jtag_parse "$_jl" 0x40800123 "no ELF" 0 0x40800100)" || _fail "(20) parse with a wrong clk address returned non-zero"
    printf '%s\n' "$_jp" | $GREP -qxF 'clk_result: not read (no 8-word line at 0x40800100)' || _fail "(20) wrong clk address not reported: $_jp"
    #  refused (other board) prints neither pcr nor clk_result
    _jfix "Info : esp_usb_jtag: serial (F4:12:FA:5B:4A:58)" 0x40800abc 0x40800def 000003e8 000007d0
...
LOG_DIR="${LOG_DIR:-$HOME/TOPPERS/ESP32/fmp3_esp_idf_dev/.steering/20260916-c5-arduino-plan/stage2/logs}"
...
#  outside LOG_DIR is masked only with C5_REDACT_ANYWHERE=1 said out loud.
c5_redact_guard() {
    local f="$1" abs dir_abs
    if git -C "$(dirname -- "$f")" ls-files --error-unmatch -- "$(basename -- "$f")" >/dev/null 2>&1; then
        die "C5_REDACT_ONLY refuses a git-tracked file: $f (a masked source or document is not a redacted log)"
    fi
    abs="$(realpath -m -- "$f")"; dir_abs="$(realpath -m -- "$LOG_DIR")"
    case "$abs" in
        "$dir_abs"/*) ;;
        *) [ "${C5_REDACT_ANYWHERE:-0}" = "1" ] \
               || die "C5_REDACT_ONLY refuses a file outside LOG_DIR ($LOG_DIR): $f (set C5_REDACT_ANYWHERE=1 for a log kept elsewhere)" ;;
    esac
}
if [ "${C5_REDACT_ONLY:-0}" = "1" ]; then
    [ "$#" -ge 1 ] || die "C5_REDACT_ONLY=1 needs the files to redact as arguments"
    for f in "$@"; do [ -f "$f" ] || die "not a file: $f"; done
    for f in "$@"; do c5_redact_guard "$f"; done
    c5_load_needles
    C5_FILES=("$@")
    c5_redact_on_exit
    exit "$?"
...
trap 'c5_redact_on_exit' EXIT

#  ---------------------------------------------------------------- modes
DRYRUN="${DRYRUN:-0}"; NOFLASH="${NOFLASH:-0}"; COLD="${COLD:-0}"; NORESET="${NORESET:-0}"
C5_JTAG_ON_SILENT="${C5_JTAG_ON_SILENT:-1}"; C5_JTAG_FORCE="${C5_JTAG_FORCE:-0}"
for _v_name in DRYRUN NOFLASH COLD NORESET C5_JTAG_ON_SILENT C5_JTAG_FORCE; do
    eval "_v_val=\$$_v_name"
    case "$_v_val" in
        0|1) ;;
...
if [ "$COLD" != "1" ] && { [ "$NOFLASH" != "1" ] || [ "$DRYRUN" = "1" ]; }; then
    ...
    #  literal here, and must be the C5's 0x2000 (gate 2b).
    BL_ADDR="$(c5_bootloader_addr "$BOARDS_TXT")" || die "bootloader address: $BL_ADDR"
...
#  One writing run = three esptool calls chained in the ROM download mode
#  that the identification (flash-id --after no-reset) left the chip in:
#  erase-region, write-flash, read-flash (the last one wakes the board with
#  --after hard-reset). The development-side script proved this chain on
#  this board with esptool 5.2.0.
ERASE_CMD=("$ESPTOOL" --chip esp32c5 --port "$DUT_PORT" --baud "$BAUD"
           --before no-reset --after no-reset erase-region "$ERASE_START" "$ERASE_LEN")
FLASH_CMD=("$ESPTOOL" --chip esp32c5 --port "$DUT_PORT" --baud "$BAUD"
           --before no-reset --after no-reset write-flash -z
           --flash-mode keep --flash-freq keep --flash-size keep "${WRITE_ARGS[@]}")
READBACK_CMD_PREFIX=("$ESPTOOL" --chip esp32c5 --port "$DUT_PORT" --baud "$BAUD"
           --before no-reset --after hard-reset read-flash "$ERASE_START" "$ERASE_LEN")

print_write_plan() {
    local i
    say "write plan, step 1: erase-region $ERASE_START $ERASE_LEN (flash 0x0-0x1FFF, the Direct Boot magic; idempotent)"
    printf '    '; printf '%q ' "${ERASE_CMD[@]}"; printf '\n'
    say "write plan, step 2: write-flash (address  sha256  size  file):"
    ...
    say "write plan, step 3: read-flash $ERASE_START $ERASE_LEN -> must be all 0xFF (the erase's read-back); --after hard-reset wakes the board"
    printf '    '; printf '%q ' "${READBACK_CMD_PREFIX[@]}" '<tmpfile>'; printf '\n'
}

#  ---------------------------------------------------------------- 2. output paths
#  LOG_DIR: set above the redact-only block.
OUT="${OUT:-$LOG_DIR/c5-capture-$(date +%Y%m%d-%H%M%S).log}"
BASE="${OUT%.log}"
...
if [ "$COLD" = "1" ]; then
    ...
    mkdir -p "$(dirname "$OUT")"
    C5_FILES+=("$IDENT_LOG")
    if ! "$ESPTOOL" --chip esp32c5 --port "$DUT_PORT" --no-stub --after "$IDENT_AFTER" flash-id >"$IDENT_LOG" 2>&1; then
        sed 's/^/    /' "$IDENT_LOG" >&2
    ...
    #  BASE MAC is the 48-bit one; the "MAC:" line on C5 is the EUI-64 form.
  ...
  expected $DUT_MAC (is a different board on this port?)"
    #  Full chip string, fixed-string: "ESP32-C5" alone would also accept ESP32-C51.
...
if [ "$NOFLASH" != "1" ]; then
    ...
    C5_FILES+=("$FLASH_LOG")
    : > "$FLASH_LOG"
    #  2a. erase 0x0-0x1FFF (Direct Boot magic). The chip is in the ROM
    #  download mode since the identification; --before no-reset keeps it there.
    echo "## 2a. erase-region $ERASE_START $ERASE_LEN" >> "$FLASH_LOG"
    if ! "${ERASE_CMD[@]}" >>"$FLASH_LOG" 2>&1; then
        tail -20 "$FLASH_LOG" | sed 's/^/    /' >&2
        echo "# erase-region $ERASE_START $ERASE_LEN FAILED (rc!=0) before any image was written; flash 0x0-0x1FFF is UNDEFINED. See $FLASH_LOG" >> "$SHA_TXT"
        die "erase-region failed; see $FLASH_LOG"
    fi
    say "erase 0x0-0x1FFF OK (esptool rc=0; read-back follows the write)"
    #  2b. the four images.
    echo "## 2b. write-flash" >> "$FLASH_LOG"
    ...
    #  2c. read 0x0-0x1FFF back: all 0xFF, or the old Direct Boot image may
    #  still win the ROM's choice. The last esptool call, so --after
    #  hard-reset wakes the board (the monitor resets it once more).
    RB_DIR="$(mktemp -d)"; RB="$RB_DIR/sector0-1.bin"
    echo "## 2c. read-flash $ERASE_START $ERASE_LEN (read-back of the erase)" >> "$FLASH_LOG"
    if ! "${READBACK_CMD_PREFIX[@]}" "$RB" >>"$FLASH_LOG" 2>&1; then
        tail -20 "$FLASH_LOG" | sed 's/^/    /' >&2
        echo "# read-flash $ERASE_START $ERASE_LEN FAILED (rc!=0) after the write; the erase is UNVERIFIED (images written, Hash of data verified x$N). See $FLASH_LOG" >> "$SHA_TXT"
        rm -rf "$RB_DIR"
        die "read-flash (erase read-back) failed; see $FLASH_LOG"
    fi
    RB_SIZE="$(stat -c %s "$RB")"
    NONFF="$(tr -d '\377' < "$RB" | wc -c)"
    HEAD16="$(od -An -tx1 -N16 "$RB" | tr -s ' ' | sed 's/^ //')"
    rm -rf "$RB_DIR"
    if [ "$RB_SIZE" -ne "$((ERASE_LEN))" ] || [ "$NONFF" -ne 0 ]; then
        echo "# erase read-back FAILED: read $RB_SIZE bytes (expected $((ERASE_LEN))), non-0xFF bytes $NONFF, first 16: $HEAD16 -- the Direct Boot magic may still be in place; images written (Hash of data verified x$N). See $FLASH_LOG" >> "$SHA_TXT"
        die "erase read-back: $NONFF non-0xFF bytes in 0x0-0x1FFF (size $RB_SIZE; first 16: $HEAD16); see $FLASH_LOG"
    fi
    say "erase read-back OK (0x0-0x1FFF all 0xFF, $RB_SIZE bytes, first 16: $HEAD16)"
    {
        echo "# erased by this run: $ERASE_START +$ERASE_LEN (0x0-0x1FFF), read back all 0xFF ($RB_SIZE bytes, first 16: $HEAD16)"
...
#  ---------------------------------------------------------------- 7. markers
MARKER_LINES="$(c5_count_markers "$OUT")"
printf '%s\n' "$MARKER_LINES" | sed 's/^/[C5] /'
printf '%s\n' "$MARKER_LINES" >> "$SHA_TXT"
#  EXTRA_MARKERS (fixed strings; nothing when unset).
EXTRA_LINE="$(c5_count_extra "$OUT")"
if [ -n "$EXTRA_LINE" ]; then
    say "$EXTRA_LINE"
    printf '%s\n' "$EXTRA_LINE" >> "$SHA_TXT"
fi
#  APM readback evidence (register values only) for the stage 4 control.
if [ -n "$(c5_apm_lines "$OUT")" ]; then
    { echo "# apm readback lines:"; c5_apm_lines "$OUT" | sed 's/^/#   /'; } >> "$SHA_TXT"
fi
say "record -> $SHA_TXT"
...
if [ "$(c5_jtag_wanted "$COLD" "$HEARTBEAT" "$C5_JTAG_FORCE" "$DRYRUN" "$C5_JTAG_ON_SILENT")" = "yes" ]; then
    _reason=silent-cold
    [ "$C5_JTAG_FORCE" = "1" ] && _reason=forced
    c5_jtag_probe "$_reason"
else
    say "jtag: probe not run (COLD=$COLD heartbeat=$HEARTBEAT C5_JTAG_FORCE=$C5_JTAG_FORCE C5_JTAG_ON_SILENT=$C5_JTAG_ON_SILENT); it runs on a silent cold run or with C5_JTAG_FORCE=1"
fi
exit 0
